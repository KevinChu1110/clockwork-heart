...
func _init() -> void:
	print("=== RUNNING CLOUD CRANE PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("crane")
	print("Slot paths resolved for crane:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/crane/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to crane paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 CLOUD CRANE PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
