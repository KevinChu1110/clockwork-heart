...
def eval_skill_candidate(ear_dy=0, tail_factor=1.0, staff_deg=-45, staff_hand=(86, 68)):
    shifts = {
        "ear_l": (-3, ear_dy), "ear_r": (4, ear_dy), "forehead": (1, int(round(ear_dy * 0.8))),
        "eye_l": (0, int(round(ear_dy * 0.7))), "eye_r": (2, int(round(ear_dy * 0.7))), "snout": (3, int(round(ear_dy * 0.9))),
        "neck": (2, -3), "core": (2, -3), "shoulder_l": (-3, -4), "shoulder_r": (5, -5),
        "key_mount": (-2, -3), "key_top": (-3, -3), "key_bot": (-3, -3),
        "pelvis": (1, -2), "hip_l": (-2, -2), "hip_r": (2, -2),
        "tail_base": (-2, -4), 
        "tail_mid": (int(round(-10 * tail_factor)), int(round(-12 * tail_factor))), 
        "tail_tip": (int(round(-18 * tail_factor)), int(round(-18 * tail_factor))),
    ...
    if 108 <= h <= 111 and max_y == 117 and diff_px >= 6000 and resid <= 54.84 and len(comps) == 1:
        print(f"SKILL MATCH: ear_dy={ear_dy}, tf={tail_factor:.2f}, deg={staff_deg}, hand={staff_hand} -> "
              f"top_y={min_y:2d}, bot_y={max_y:2d}, h={h:3d} | diff={diff_px:5d} | "
              f"resid={resid:.2f} | comps={len(comps)} | sh0={sh_rows[0]}")

for edy in [-1, 0, 1]:
    for tf in [1.0, 1.1, 1.2]:
        for deg in [-42, -45, -48]:
            for hy in [66, 68, 70]:
                eval_skill_candidate(ear_dy=edy, tail_factor=tf, staff_deg=deg, staff_hand=(86, hy))
