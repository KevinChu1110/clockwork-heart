...
func _init() -> void:
	print("=== RUNNING COLOSSUS ELEPHANT PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("elephant")
	print("Slot paths resolved for elephant:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/elephant/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to elephant paperdoll slice: %s" % [sid, path])
			failed = true
	...
	var comp := PaperdollRenderer.build_composite_image("elephant")
	...
	if failed:
		...
		print("=== ALL 7 COLOSSUS ELEPHANT PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
