#!/usr/bin/env python3
"""
tools/make_berserker_comparison_8x.py
Generates an 8x scaled comparison board on magenta:
Left: head_unit (head_iron_bear_stock.png)
Middle: first costume (costume_ironclad_overalls.png)
Right: new second costume (costume_berserker_cuirass.png)
...
def make_board(out_path="/tmp/bear_costume_comparison_8x.png"):
    p_head = f"{BEAR_DIR}/head_unit/head_iron_bear_stock.png"
    p_c1 = f"{BEAR_DIR}/costume/costume_ironclad_overalls.png"
    p_c2 = f"{BEAR_DIR}/costume/costume_berserker_cuirass.png"
    ...
    curr_x = pad
    # 1. Head
    d.text((curr_x, 15), "1. Head Unit (Ref)", fill=(255, 220, 80, 255))
    board.paste(m_head, (curr_x, header))
    curr_x += m_head.width + pad

    # 2. Costume 1
    d.text((curr_x, 15), "2. Costume 1 Overalls (Ref)", fill=(255, 220, 80, 255))
    board.paste(m_c1, (curr_x, header))
    curr_x += m_c1.width + pad

    # 3. Costume 2
    d.text((curr_x, 15), "3. Costume 2 Berserker (New)", fill=(255, 220, 80, 255))
