...
func _init() -> void:
	print("=== RUNNING THE IRON BEAR PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("bear")
	print("Slot paths resolved for bear:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/bear/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to bear paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 THE IRON BEAR PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
