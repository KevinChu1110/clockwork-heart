...
func _init() -> void:
	print("=== RUNNING STEAM PENGUIN PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("penguin")
	print("Slot paths resolved for penguin:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/penguin/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to penguin paperdoll slice: %s" % [sid, path])
			failed = true
	...
	var comp := PaperdollRenderer.build_composite_image("penguin")
	...
	if failed:
		...
		print("=== ALL 7 STEAM PENGUIN PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
