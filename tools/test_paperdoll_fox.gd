...
func _init() -> void:
	print("=== RUNNING FOX PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("fox")
	print("Slot paths resolved for fox:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/fox/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to fox paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 FOX PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
