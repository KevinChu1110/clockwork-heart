...
func _init() -> void:
	print("=== RUNNING BOAR PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("boar")
	print("Slot paths resolved for boar:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/boar/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to boar paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 BOAR PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
