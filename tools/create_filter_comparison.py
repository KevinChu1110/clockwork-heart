...
def main():
    ...
    path_bear = os.path.join(screenshots_dir, "proof_wardrobe_filter_bear.png")
    path_out = os.path.join(screenshots_dir, "proof_wardrobe_filter_comparison.png")

    if not os.path.exists(path_all) or not os.path.exists(path_bear):
        print(f"Error: Missing input screenshots {path_all} or {path_bear}")
        return

    img_all = Image.open(path_all).convert("RGB")
    img_bear = Image.open(path_bear).convert("RGB")

    w, h = img_all.size
    # 縮小為 960x540 並排
    target_w, target_h = 960, 540
    resized_all = img_all.resize((target_w, target_h), Image.Resampling.LANCZOS)
    resized_bear = img_bear.resize((target_w, target_h), Image.Resampling.LANCZOS)
    ...
    canvas.paste(resized_bear, (target_w + 10, banner_h + 10))
