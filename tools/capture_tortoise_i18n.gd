extends SceneTree
## 玄機龜玩家可見名稱六語系落地實機截圖腳本 (tortoise-i18n)
## 驗收重點：
## 1. 創角擴充分頁選玄機龜，展示【玄機龜】、【法師】、【天元道場玄機護甲】、【原廠青銅古翠綠】在 zh_TW, en, ja 下即時切換。
## 2. 衣櫥選用玄機龜外裝／塗裝標籤，在大廳背景與底部 Dock 連動下（0-QA25），在 zh_TW, en, ja 下即時切換。

const OUT_DIR := "/opt/side/bravesoul-game/proofs/tortoise-i18n"
const CROPS_DIR := "/opt/side/bravesoul-game/proofs/tortoise-i18n/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行玄機龜六語系落地實機截圖腳本 (tortoise-i18n) ──")
	_step = 1
	_wait = 0
...
func _process(_delta: float) -> bool:
	...
	match _step:
		1, 2, 3:
			# 步驟 1~3: 創角介面 (PaperdollSelectDemo) 擴充分頁選玄機龜 (zh_TW, en, ja)
			var loc_idx := _step - 1
			...
			if _wait == 1:
				...
				var path := "%s/proof_creation_tortoise_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 創角介面選玄機龜 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
		...
		4, 5, 6:
			# 步驟 4~6: 大廳開啟衣櫥 (WardrobeDialog)，篩選「龜」展示工裝與塗裝標籤，背景大廳與 Dock 同步切換語系 (0-QA25)
			var loc_idx := _step - 4
			var code: String = TEST_LOCALES[loc_idx]
			if _wait == 1:
				if _loc_node:
				...
				if _current_node and _current_node.has_method("open_wardrobe"):
					...
					if wardrobe and wardrobe.has_method("set_race_filter"):
						wardrobe.call("set_race_filter", "tortoise")
			elif _wait >= 40:
				var path := "%s/proof_wardrobe_tortoise_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 大廳連動衣櫥龜族標籤 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step += 1
...
func _generate_crops() -> void:
	...
	for code in TEST_LOCALES:
		var c_path := "%s/proof_creation_tortoise_%s.png" % [OUT_DIR, code]
		...
		var w_path := "%s/proof_wardrobe_tortoise_%s.png" % [OUT_DIR, code]
