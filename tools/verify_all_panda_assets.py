#!/usr/bin/env python3
"""
verify_all_panda_assets.py
Comprehensive verification script for Porcelain Panda (瓷韻熊貓) official assets suite.
Matches test patterns from verify_all_frog_assets.py and verify_all_elephant_assets.py.
"""
...
def verify():
    print("=== VERIFYING ALL PORCELAIN PANDA OFFICIAL ASSETS ===")
    all_ok = True

    required_assets = [
        # Category 1: 官網英雄圖 / 品牌立牌
        ("branding/char_panda.png", [(400, 840), (800, 1680), (1344, 1680)], ["RGB", "RGBA"]),
        ("web/media/hero/char_panda.png", [(400, 840), (1344, 1680)], ["RGB", "RGBA"]),
        ("docs/art/char_panda_candidate_400x840.png", (400, 840), ["RGB", "RGBA"]),
        ("docs/art/porcelain_panda_concept.png", [(928, 1152), (1344, 1680)], ["RGB", "RGBA"]),

        # Category 2: 戰鬥特寫
        ("game/assets/sprites/player/panda_battle.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/proof_panda_idle_vs_battle.png", (256, 128), ["RGBA"]),

        # Category 3: 行走動畫
        ("game/assets/sprites/player/panda_walk_0.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_1.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_2.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_3.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_0_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_1_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_2_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/panda_walk_3_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/proof_panda_walk_cycle.png", (512, 128), ["RGBA"]),

        # Mirror idle assets
        ("game/assets/sprites/player/panda_idle.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/panda_idle_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/party/panda_idle.png", (128, 128), ["RGBA"]),
        ("web/media/hero/panda_idle.png", (128, 128), ["RGBA"]),

        # Category 4: HUD 戰鬥頭像 與 對話框半身像
        ("game/assets/sprites/portraits/panda.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/portraits/porcelain_panda.png", (384, 480), ["RGBA"]),
    ]
    ...
    standee = Image.open(f"{BRANDING_DIR}/char_panda.png")
    ...
    base_idle = Image.open(f"{PLAYER_DIR}/panda_idle_x3.png").convert("RGBA")
    w_x3 = [Image.open(f"{PLAYER_DIR}/panda_walk_{i}_x3.png").convert("RGBA") for i in range(4)]
    ...
    for i in range(4):
        for dx in range(-8, 9):
            for dy in range(-8, 9):
                ...
                if diff is None:
                    ...
                    all_ok = False
        print(f"  ✓ panda_walk_{i}_x3 passed 4b-4: guaranteed not pure translation under any offset")
        ...
        if min_diff <= 300:
            ...
            print(f"  ✓ panda_walk_{i}_x3 passed 4b-7: articulation diff={min_diff}px > 300px")
    ...
    for i in range(4):
        f_shadow = [sum(1 for x in range(128) if cast(tuple[int, int, int, int], w_x3[i].getpixel((x, y)))[3] > 20) for y in range(118, 128)]
        if f_shadow != base_shadow:
            print(f"  ❌ Frame {i} shadow {f_shadow} differs from base {base_shadow}!")
            all_ok = False
        else:
            print(f"  ✓ panda_walk_{i}_x3 passed 4b-5: shadow row counts {f_shadow} 100% match idle baseline")

    # 5. Rule 4b-6: Battle Sprite
    print("\n--- [5/6] Rule 4b-6 Battle Sprite Metrics ---")
    battle = Image.open(f"{PLAYER_DIR}/panda_battle.png").convert("RGBA")
    ...
    ch_legs = sum(1 for y in range(32) for x in range(128) if any(c > 0 for c in cast(tuple[int, int, int, int], diff_legs.getpixel((x, y)))))
    ...
    hud = Image.open(f"{PORTRAITS_DIR}/panda.png").convert("RGBA")
    h_bbox = hud.getbbox()
    ...
    bust = Image.open(f"{PORTRAITS_DIR}/porcelain_panda.png").convert("RGBA")
    ...
    if all_ok:
        print("\n🎉 ALL VERIFICATION CHECKS PASSED FOR PORCELAIN PANDA!")
        return True
    else:
        print("\n❌ SOME CHECKS FAILED!")
        return False
