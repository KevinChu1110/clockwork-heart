extends SceneTree
## 瓷韻熊貓玩家可見名稱六語系落地實機截圖腳本 (panda-i18n)
## 驗收重點：
## 1. 創角擴充分頁選瓷韻熊貓，展示【瓷韻熊貓】、【武術家】、【禪道學徒生漆長袍】、【羊脂白瓷生漆塗裝】在 zh_TW, en, ja 下即時切換。
## 2. 衣櫥選用瓷韻熊貓外裝／塗裝標籤，在大廳背景與底部 Dock 連動下（0-QA25），在 zh_TW, en, ja 下即時切換。

const OUT_DIR := "/opt/side/bravesoul-game/proofs/panda-i18n"
const CROPS_DIR := "/opt/side/bravesoul-game/proofs/panda-i18n/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行瓷韻熊貓六語系落地實機截圖腳本 (panda-i18n) ──")
	_step = 1
	_wait = 0
...
func _process(_delta: float) -> bool:
	...
	match _step:
		1, 2, 3:
			# 步驟 1~3: 創角介面 (PaperdollSelectDemo) 擴充分頁選瓷韻熊貓 (zh_TW, en, ja)
			var loc_idx := _step - 1
			...
			if _wait == 1:
				...
				var path := "%s/proof_creation_panda_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 創角介面選瓷韻熊貓 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
		...
		4, 5, 6:
			# 步驟 4~6: 大廳開啟衣櫥 (WardrobeDialog)，篩選「貓」展示長袍與塗裝標籤，背景大廳與 Dock 同步切換語系 (0-QA25)
			var loc_idx := _step - 4
			var code: String = TEST_LOCALES[loc_idx]
			if _wait == 1:
				if _loc_node:
				...
				if _current_node and _current_node.has_method("open_wardrobe"):
					...
					if wardrobe and wardrobe.has_method("set_race_filter"):
						wardrobe.call("set_race_filter", "panda")
			elif _wait >= 40:
				var path := "%s/proof_wardrobe_panda_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 大廳連動衣櫥熊貓標籤 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step += 1
...
func _generate_crops() -> void:
	# 1. 裁切創角右側資訊面板文字區 (X:760~1260, Y:100~580) 驗證四詞
	for code in TEST_LOCALES:
		var c_path := "%s/proof_creation_panda_%s.png" % [OUT_DIR, code]
		if FileAccess.file_exists(c_path):
			...
			if img and not img.is_empty():
				...
				var badge_crop := img.get_region(Rect2i(760, 100, 400, 120))
				var badge_p := "%s/crop_hero_badge_%s.png" % [CROPS_DIR, code]
				badge_crop.save_png(badge_p)
				print("  [Crop] 創角英雄徽章特寫 [%s]: %s" % [code, badge_p])

		# 2. 裁切衣櫥卡片標籤與左側徽章區 (X:220~920, Y:150~600)
		var w_path := "%s/proof_wardrobe_panda_%s.png" % [OUT_DIR, code]
		if FileAccess.file_exists(w_path):
			...
			if img and not img.is_empty():
				...
				var badge_crop := img.get_region(Rect2i(100, 120, 350, 100))
				var badge_p := "%s/crop_wardrobe_badge_%s.png" % [CROPS_DIR, code]
				badge_crop.save_png(badge_p)
				print("  [Crop] 衣櫥左側徽章特寫 [%s]: %s" % [code, badge_p])

				# 3. 底部 Dock 區 (X:0~1280, Y:620~720) 驗證 0-QA25
