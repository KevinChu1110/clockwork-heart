...
def process_image(name):
    path = f"game/assets/sprites/player/showcase/{name}_idle_hd.png"
    im = Image.open(path)
    arr = np.array(im, dtype=np.float32)
    h, w, _ = arr.shape
    
    # Measure corner/border background
    ...
    print(f"\nProcessing {name} ({w}x{h}): bg_mean={bg_mean.astype(int).tolist()}")
    ...
    # Check for isolated pure-background holes (e.g. between legs for bear, crane, tiger)
    if name in ["bear", "crane", "macaque", "tiger"]:
    ...
    out_im.save(path)
    print(f"  Successfully updated {path}: mode={out_im.mode}, size={out_im.size}")
...
races = ["bear", "boar", "crane", "fox", "lion", "macaque", "penguin", "tiger"]
for r in races:
    process_image(r)

print("\n--- Summary self-check for all 9 showcase files ---")
for p in sorted(glob.glob("game/assets/sprites/player/showcase/*_idle_hd.png")):
    im = Image.open(p)
    corners = [(0, 0), (im.width - 1, 0), (0, im.height - 1), (im.width - 1, im.height - 1)]
    c_pixels = [im.getpixel(c) for c in corners]
    alphas = [int(p_val[3]) if isinstance(p_val, tuple) and len(p_val) > 3 else -1 for p_val in c_pixels]
    all_zero = all(a == 0 for a in alphas)
    print(f"{os.path.basename(p)}: mode={im.mode}, size={im.size}, corners_alpha={alphas}, corners_ok={all_zero}")
