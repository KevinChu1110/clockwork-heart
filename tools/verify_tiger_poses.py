#!/usr/bin/env python3
"""
verify_tiger_poses.py
Comprehensive verification test suite for Tiger combat action poses (128x128 RGBA).
Mirrors verify_lion_poses.py and validate_final_6_poses.py.
"""
import os
import hashlib
...
POSES_DIR = f"{REPO_ROOT}/game/assets/sprites/player/poses/tiger"
EXPECTED_SHADOW = [49, 55, 57, 55, 49, 37, 1, 0, 0, 0]
POSES = ['idle', 'attack', 'hit', 'recover', 'skill', 'telegraph']
lut = [0] * 11 + [1] * 245

print("=== VERIFYING EMBER TIGER COMBAT POSES (128x128 RGBA) ===")
...
idle_h = images['idle'].getbbox()[3] - images['idle'].getbbox()[1] # type: ignore
for p in POSES:
    bbox = images[p].getbbox()
...
print("\n=== ALL AUTOMATED METRIC VERIFICATIONS COMPLETED ===")
