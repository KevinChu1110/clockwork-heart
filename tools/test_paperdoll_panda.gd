...
func _init() -> void:
	print("=== RUNNING PORCELAIN PANDA PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("panda")
	print("Slot paths resolved for panda:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/panda/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to panda paperdoll slice: %s" % [sid, path])
			failed = true
	...
	var comp := PaperdollRenderer.build_composite_image("panda")
	...
	if failed:
		...
		print("=== ALL 7 PORCELAIN PANDA PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
