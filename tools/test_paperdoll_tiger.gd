...
func _init() -> void:
	print("=== RUNNING TIGER PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("tiger")
	print("Slot paths resolved for tiger:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/tiger/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to tiger paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 TIGER PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
