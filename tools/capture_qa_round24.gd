extends SceneTree
## QA Round 24: 大廳全景六語系切換實機截圖與回歸驗收腳本
## 執行方式：xvfb-run -a godot --path game --rendering-driver opengl3 -s res://../tools/capture_qa_round24.gd

const FRAMES := 8
const OUT_DIR := "/opt/side/bravesoul-game/proofs/qa_round24"
...
func _initialize() -> void:
	...
	print("── 開始執行 QA Round 24 大廳全景六語系實機截圖 ──")
...
func _run_capture() -> void:
	...
	for code in LOCALES:
		...
		print("  ok 已儲存 [%s] 大廳全景實機截圖: %s" % [code, out_path])
		_verify_texts(code)

	# 復原繁中
	_loc_node.call("set_locale", "zh_TW")
	print("── QA Round 24 大廳六語系截圖全數完成 ──")
	quit(0)
