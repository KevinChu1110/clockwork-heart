#!/usr/bin/env python3
"""
verify_all_frog_assets.py
Comprehensive verification script for Spring-Leg Frog (碧簧蛙) official assets suite.
Matches test patterns from verify_all_elephant_assets.py and verify_all_tortoise_assets.py.
"""
...
def verify():
    print("=== VERIFYING ALL SPRING-LEG FROG OFFICIAL ASSETS ===")
    all_ok = True

    required_assets = [
        # Category 1: 官網英雄圖 / 品牌立牌
        ("branding/char_frog.png", [(400, 840), (800, 1680), (1344, 1680)], ["RGB", "RGBA"]),
        ("web/media/hero/char_frog.png", [(400, 840), (1344, 1680)], ["RGB", "RGBA"]),
        ("docs/art/char_frog_candidate_400x840.png", (400, 840), ["RGB", "RGBA"]),
        ("docs/art/spring_frog_concept.png", [(928, 1152), (1344, 1680)], ["RGB", "RGBA"]),

        # Category 2: 戰鬥特寫
        ("game/assets/sprites/player/frog_battle.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/proof_frog_idle_vs_battle.png", (256, 128), ["RGBA"]),

        # Category 3: 行走動畫
        ("game/assets/sprites/player/frog_walk_0.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_1.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_2.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_3.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_0_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_1_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_2_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/frog_walk_3_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/proof_frog_walk_cycle.png", (512, 128), ["RGBA"]),

        # Mirror idle assets
        ("game/assets/sprites/player/frog_idle.png", (64, 64), ["RGBA"]),
        ("game/assets/sprites/player/frog_idle_x3.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/player/party/frog_idle.png", (128, 128), ["RGBA"]),
        ("web/media/hero/frog_idle.png", (128, 128), ["RGBA"]),

        # Category 4: HUD 戰鬥頭像 與 對話框半身像
        ("game/assets/sprites/portraits/frog.png", (128, 128), ["RGBA"]),
        ("game/assets/sprites/portraits/spring_frog.png", (384, 480), ["RGBA"]),
    ]
    ...
    standee = Image.open(f"{BRANDING_DIR}/char_frog.png")
    ...
    base_idle = Image.open(f"{PLAYER_DIR}/frog_idle_x3.png").convert("RGBA")
    w_x3 = [Image.open(f"{PLAYER_DIR}/frog_walk_{i}_x3.png").convert("RGBA") for i in range(4)]
    ...
    for i in range(4):
        for dx in range(-8, 9):
            for dy in range(-8, 9):
                ...
                if diff is None:
                    ...
                    all_ok = False
        print(f"  ✓ frog_walk_{i}_x3 passed 4b-4: guaranteed not pure translation under any offset")
        ...
        if min_diff <= 300:
            ...
            print(f"  ✓ frog_walk_{i}_x3 passed 4b-7: articulation diff={min_diff}px > 300px")

    # 4. Rule 4b-5: Shadow Consistency
    print("\n--- [4/6] Rule 4b-5 Walk Ground Shadow Consistency ---")
    base_shadow = [sum(1 for x in range(128) if base_idle.getpixel((x, y))[3] > 20) for y in range(118, 128)]
    for i in range(4):
        f_shadow = [sum(1 for x in range(128) if w_x3[i].getpixel((x, y))[3] > 20) for y in range(118, 128)]
        if f_shadow != base_shadow:
            print(f"  ❌ Frame {i} shadow {f_shadow} differs from base {base_shadow}!")
            all_ok = False
        else:
            print(f"  ✓ frog_walk_{i}_x3 passed 4b-5: shadow row counts {f_shadow} 100% match idle baseline")

    # 5. Rule 4b-6: Battle Sprite
    print("\n--- [5/6] Rule 4b-6 Battle Sprite Metrics ---")
    battle = Image.open(f"{PLAYER_DIR}/frog_battle.png").convert("RGBA")
    ...
    hud = Image.open(f"{PORTRAITS_DIR}/frog.png").convert("RGBA")
    h_bbox = hud.getbbox()
    ...
    bust = Image.open(f"{PORTRAITS_DIR}/spring_frog.png").convert("RGBA")
    ...
    if all_ok:
        print("\n🎉 ALL VERIFICATION CHECKS PASSED FOR SPRING-LEG FROG!")
        return True
    else:
        print("\n❌ SOME CHECKS FAILED!")
        return False
