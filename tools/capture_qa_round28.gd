extends SceneTree
## QA Round 28: 鋼岳象（第十一族）全套＋碧箸蛙（第十二族）骨架合併後探索性 QA 驗收截圖腳本
## 涵蓋：
## 1. 創角畫面 (PaperdollSelectDemo)：鋼岳象第一套外裝【巨輪工坊厚鋼工裝】與第一套塗裝【原廠巨輪工坊黃銅原金】
## 2. 創角畫面：切換選用鋼岳象第二套外裝【鋼岳要塞重裝戰鎧】與塗裝【高爐鎢鋼淬火黑】，驗證 512 舞台與創角確認
## 3. 衣櫥 (WardrobeDialog) 篩選「全部」驗證全族清單
## 4. 衣櫥篩選「鋼岳象」，選戴【鋼岳要塞重裝戰鎧】與【高爐鎢鋼淬火黑】，即時預覽與選中態卡片
## 5. 衣櫥切換回【巨輪工坊厚鋼工裝】與【原廠巨輪工坊黃銅原金】對照，驗證無污染
## 6. 手遊大廳 (MobileLobby) 驗證 512 高清紙娃娃與六語系 (zh_TW, zh_CN, en, ja, ko, es) 即時刷新
## 7. 大廳連動商城即時刷新 (en 英文環境，0-QA25 檢查)
## 8. 戰鬥畫面 (BattleView) 鋼岳象穿戴【鋼岳要塞重裝戰鎧】出戰
## 9. 戰鬥名稱備援對照（空名 fallback 鋼岳象族名與打擊事件戰鬥日誌）
## 10. 碧箸蛙（第十二族 frog）骨架資料表載入與安全 Fallback 驗證 (DevPaperdollPreview)
## 11. 特寫 Crops 產出

const OUT_DIR := "/opt/side/bravesoul-game/proofs/qa_round28"
const CROPS_DIR := "/opt/side/bravesoul-game/proofs/qa_round28/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行 QA Round 28 鋼岳象全套＋碧箸蛙骨架探索性實機截圖 ──")
	_step = 1
	_wait = 0
...
func _process(_delta: float) -> bool:
	...
	match _step:
		1:
			# 步驟 1: 創角介面 (PaperdollSelectDemo) 選中鋼岳象第一套外裝與原廠黃銅原金
			if _wait == 1:
				if _loc_node:
					_loc_node.call("set_locale", "zh_TW")
				if _gs:
					_gs.call("reset_new_game", "elephant")
				...
				var path := "%s/proof_01_creation_elephant_default.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [1/15] 創角選用鋼岳象第一套外裝【巨輪工坊厚鋼工裝】與【原廠巨輪工坊黃銅原金】: %s" % path)
		...
		2:
			# 步驟 2: 創角介面切換至第二套外裝【鋼岳要塞重裝戰鎧】與塗裝【高爐鎢鋼淬火黑】
			if _wait == 5:
				if _current_node:
					var data: Dictionary = _current_node.get("RACES_DATA").get("elephant", {})
					var costumes: Array = data.get("costumes", [])
					var fortress_idx := 0
					for idx in range(costumes.size()):
						if costumes[idx].get("id") == "costume_colossus_bastion_plate":
							fortress_idx = idx
							break
					var chassis_list: Array = data.get("chassis", [])
					var tungsten_idx := 0
					for idx in range(chassis_list.size()):
						if chassis_list[idx].get("id") == "paint_tungsten_iron":
							tungsten_idx = idx
							break

					_current_node.set("_costume_index", fortress_idx)
					_current_node.set("_chassis_index", tungsten_idx)
					_current_node.call("_apply_current_selections")
			elif _wait >= 30:
				var path := "%s/proof_02_creation_elephant_fortress.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [2/15] 創角切換為第二套外裝【鋼岳要塞重裝戰鎧】與【高爐鎢鋼淬火黑】: %s" % path)
		...
		3:
			...
			if _wait == 1:
				...
				_save_screenshot(path)
				print("  ✓ [3/15] 衣櫥篩選「全部」驗證全族清單: %s" % path)
				_step = 4
				_wait = 0

		4:
			# 步驟 4: 衣櫥切換為「鋼岳象」篩選，選戴【鋼岳要塞重裝戰鎧】與【高爐鎢鋼淬火黑】
			if _wait == 5:
				if _current_node and _current_node.has_method("set_race_filter"):
					_current_node.call("set_race_filter", "elephant")
			elif _wait == 15:
				...
				_current_node.set("selected_costume_id", "costume_colossus_bastion_plate")
				_current_node.set("selected_chassis_id", "paint_tungsten_iron")
				...
				var path := "%s/proof_04_wardrobe_elephant_fortress_equipped.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [4/15] 衣櫥選用【鋼岳要塞重裝戰鎧】與【高爐鎢鋼淬火黑】: %s" % path)
				_step = 5
				_wait = 0

		5:
			# 步驟 5: 衣櫥切換回【巨輪工坊厚鋼工裝】與【原廠巨輪工坊黃銅原金】對照
			if _wait == 5:
				var scroll: ScrollContainer = _current_node.find_child("FilterScroll", true, false) as ScrollContainer
				if scroll:
					var hbar := scroll.get_h_scroll_bar()
				...
				if _current_node:
					...
					var target_ch_idx := 0
					for idx in range(displayed_ch.size()):
						if displayed_ch[idx].get("id") == "paint_elephant_brass":
							target_ch_idx = idx
							break

					_current_node.set("costume_index", target_c_idx)
					_current_node.set("chassis_index", target_ch_idx)
					_current_node.set("selected_costume_id", "costume_cog_workshop_overalls")
					_current_node.set("selected_chassis_id", "paint_elephant_brass")
					if _current_node.has_method("_update_card_selection_states"):
						_current_node.call("_update_card_selection_states")
					if _current_node.has_method("_update_preview"):
						_current_node.call("_update_preview")
				...
				var path := "%s/proof_05_wardrobe_elephant_default_equipped.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [5/15] 衣櫥切換為【巨輪工坊厚鋼工裝】對照實機圖: %s" % path)
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 6
		...
		6:
			# 步驟 6: 建立手遊大廳 (MobileLobby)，套用鋼岳象新套裝
			if _wait == 1:
				if _gs:
					_gs.call("reset_new_game", "elephant")
					_gs.set("player_race", "elephant")
					_gs.set("player_name", "鋼岳象")
				...
				if _current_node and _current_node.has_method("_hero_display_tex"):
					...
					print("    大廳鋼岳象英雄展示貼圖寬度: %d (>= 256 為 512 高清合成，嚴格遵守 lobby-no-128)" % w)
				_step = 7
				_wait = 0
		...
		7, 8, 9, 10, 11, 12:
			...
			if _wait == 1:
				...
				var path := "%s/proof_06_lobby_elephant_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/15] 已截取大廳鋼岳象 [%s] 語系實機截圖: %s" % [loc_idx + 6, code, path])
		...
		13:
			...
			if _wait == 1:
				...
				_save_screenshot(path)
				print("  ✓ [12/15] 已截取大廳連動商城即時刷新 (en): %s" % path)
		...
		14:
			# 步驟 14: 戰鬥畫面 (BattleView) 鋼岳象穿戴【鋼岳要塞重裝戰鎧】出戰
			if _wait == 1:
				if _gs:
					_gs.call("reset_new_game", "elephant")
					_gs.set("player_race", "elephant")
				...
				var path := "%s/proof_08_battle_elephant_combat.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [13/15] 已截取戰鬥畫面鋼岳象出戰: %s" % path)
		...
		15:
			# 步驟 15: 戰鬥空名備援與戰鬥日誌截圖
			if _wait >= 25:
				var path := "%s/proof_09_battle_fallback_empty_name.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [14/15] 已截取戰鬥空名備援與戰鬥日誌: %s" % path)
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 16
		...
		16:
			# 步驟 16: 碧箸蛙（第十二族 frog）骨架資料表載入與安全 Fallback 驗證
			if _wait == 1:
				...
				var path := "%s/proof_10_frog_skeleton_verification.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [15/15] 已截取碧箸蛙骨架資料表載入與安全 Fallback 驗證: %s" % path)
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 17
...
func _generate_crops() -> void:
	...
	if FileAccess.file_exists(lobby_zh):
		...
		if img and not img.is_empty():
			...
			var crop_char_p := "%s/crop_lobby_elephant_char.png" % CROPS_DIR
			crop_char.save_png(crop_char_p)
			print("  ✓ 已產出特寫: %s" % crop_char_p)

			var crop_av := img.get_region(Rect2i(30, 10, 260, 90))
			var crop_av_p := "%s/crop_lobby_elephant_avatar.png" % CROPS_DIR
			crop_av.save_png(crop_av_p)
			print("  ✓ 已產出特寫: %s" % crop_av_p)

	# 4. 戰鬥角色特寫
	var battle_p := "%s/proof_08_battle_elephant_combat.png" % OUT_DIR
	if FileAccess.file_exists(battle_p):
		var img := Image.load_from_file(battle_p)
		if img and not img.is_empty():
			var crop := img.get_region(Rect2i(100, 270, 320, 280))
			var crop_p := "%s/crop_battle_elephant_sprite.png" % CROPS_DIR
			crop.save_png(crop_p)
			print("  ✓ 已產出特寫: %s" % crop_p)

	# 5. 戰鬥名稱與日誌特寫
	var battle_log_p := "%s/proof_09_battle_fallback_empty_name.png" % OUT_DIR
