extends SceneTree
## 《發條之心》紙娃娃玄軸熊族場景節點 (PaperdollCharacter) 無頭自動化測試
## 執行方式：godot --path game --headless -s ../tools/test_paperdoll_bear_scene.gd

const PaperdollRenderer = preload("res://scripts/art/paperdoll_renderer.gd")
const PaperdollCharacter = preload("res://scripts/art/paperdoll_character.gd")
const PROOF_OUTPUT_PATH := "res://assets/sprites/player/paperdoll/bear/proof_paperdoll_bear_composite.png"


func _initialize() -> void:
	var ok := true
	print("=== 開始玄軸熊族紙娃娃實際場景節點測試 (PaperdollCharacter - bear) ===")
	...
	character.render_character("bear")
	var restored_bear_weapon: Sprite2D = character.get_slot_sprite("weapon")
	if restored_bear_weapon == null or restored_bear_weapon.texture == null or not restored_bear_weapon.visible:
		push_error("切回 bear 後未能正確恢復顯示！")
		ok = false
	else:
		print("  ✓ 斷言通過：切回 bear 後所有槽位正常恢復可見與貼圖")
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("BEAR_PAPERDOLL_SCENE_OK")
		quit(0)
	else:
		print("BEAR_PAPERDOLL_SCENE_FAIL")
		quit(1)
