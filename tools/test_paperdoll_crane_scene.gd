extends SceneTree
## 《發條之心》紙娃娃雲嵐鶴族場景節點 (PaperdollCharacter) 無頭自動化測試
## 執行方式：godot --path game --headless -s ../tools/test_paperdoll_crane_scene.gd

const PaperdollRenderer = preload("res://scripts/art/paperdoll_renderer.gd")
const PaperdollCharacter = preload("res://scripts/art/paperdoll_character.gd")
const PROOF_OUTPUT_PATH := "res://assets/sprites/player/paperdoll/crane/proof_paperdoll_crane_composite.png"


func _initialize() -> void:
	var ok := true
	print("=== 開始雲嵐鶴族紙娃娃實際場景節點測試 (PaperdollCharacter - crane) ===")
	...
	if composite_img == null or composite_img.is_empty():
		...
		if non_transparent_pixels < 1000:
			push_error("合成圖非透明像素過少 (%d < 1000)！" % non_transparent_pixels)
			ok = false
		else:
			print("  ✓ 斷言通過：合成圖具有豐富飽滿的不透明像素 (%d 像素)" % non_transparent_pixels)
	...
	character.render_character("crane")
	var restored_crane_weapon: Sprite2D = character.get_slot_sprite("weapon")
	if restored_crane_weapon == null or restored_crane_weapon.texture == null or not restored_crane_weapon.visible:
		push_error("切回 crane 後未能正確恢復顯示！")
		ok = false
	else:
		print("  ✓ 斷言通過：切回 crane 後所有槽位正常恢復可見與貼圖")
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("CRANE_PAPERDOLL_SCENE_OK")
		quit(0)
	else:
		print("CRANE_PAPERDOLL_SCENE_FAIL")
		quit(1)
