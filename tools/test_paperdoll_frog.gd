...
func _init() -> void:
	print("=== RUNNING SPRING FROG PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("frog")
	print("Slot paths resolved for frog:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/frog/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to frog paperdoll slice: %s" % [sid, path])
			failed = true
	...
	var comp := PaperdollRenderer.build_composite_image("frog")
	...
	if failed:
		...
		print("=== ALL 7 SPRING FROG PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		quit(0)
