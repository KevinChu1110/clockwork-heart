#!/usr/bin/env python3
"""
tools/verify_fox_chassis_task.py
Automated verification for Kanban Task t_21ece332:
- 0-ART27: Fox chassis has 0 ears; stacked with head_unit has exactly 2 ears.
...
HEAD_DIR = f"{REPO_ROOT}/game/assets/sprites/player/paperdoll/fox/head_unit"
...
def check_0_qa18_screenshots():
    print("=== [1/4] 0-QA18 & 0-QA15 實機截圖稽核 ===")
    targets = [
        "proof_wardrobe_fox_bare.png",
        "proof_wardrobe_fox_equipped.png"
    ]
    all_ok = True
    md5s = set()
    for fn in targets:
...
def check_0_art27_ears():
    print("=== [2/4] 0-ART27 耳朵數量與切片量測 ===")
    variants = ["paint_fox_orange", "paint_ivory_stock", "paint_emerald_glaze"]
    hd_img = Image.open(f"{HEAD_DIR}/ear_fox_radar_512.png").convert("RGBA")
    ...
    for v in variants:
        ...
        # Ears on original fox were located at y in [40..160].
        # In our headless chassis, min_y must be >= 235 (no head or ears!)
        arr512 = np.array(im512)
        head_ear_pixels = np.sum(arr512[:235, :, 3] > 0)
        assert head_ear_pixels == 0, f"{v}: found {head_ear_pixels} pixels in head/ear region (y < 235)!"
        print(f"  ✓ {v}_512: bbox={bbox512}, head/ear region pixels (y < 235) = 0 (0 耳朵合格)")
        
        # 128 version check
        bbox128 = im128.getbbox()
        assert bbox128 is not None, f"{p128} is empty!"
        min_x128, min_y128, max_x128, max_y128 = bbox128
        # In 128, ears originally reached up to y=11. Min_y must be >= 55.
        assert min_y128 >= 55, f"{v}: 128 version min_y {min_y128} < 55 (head/ears present)!"
        arr128 = np.array(im128)
        head_ear_128 = np.sum(arr128[:50, :, 3] > 0)
        assert head_ear_128 == 0, f"{v}: 128 version has {head_ear_128} pixels in head/ear region (y < 50)!"
        print(f"  ✓ {v}_128: bbox={bbox128}, head/ear region pixels (y < 50) = 0 (0 耳朵合格)")
        ...
        # In composite, the only ears are from head_unit (at y in [40..160], x in [140..393]).
        # Check that behind head_unit ears there are NO secondary chassis ears.
        chassis_behind_ears = np.sum((arr512[:160, :, 3] > 0))
...
def check_0_art26b_no_costume():
    ...
    for v in variants:
        ...
        # Check that there is no blue cape (astral cape is navy/blue: B > 120, R < 60, G < 60)
        # on the torso (y in [280..380], x in [200..320])
        torso_crop = arr[280:380, 200:320]
        navy_pixels = np.sum((torso_crop[:, :, 0] < 60) & (torso_crop[:, :, 1] < 70) & (torso_crop[:, :, 2] > 110) & (torso_crop[:, :, 3] > 150))
        assert navy_pixels == 0, f"{v}: found {navy_pixels} navy cape pixels baked into chassis!"
        print(f"  ✓ {v}: 軀幹區域零斗篷外裝烘死 (navy_pixels=0, 0-ART26b 通過)")
    print("  ✓ 0-ART26b 驗證通過！\n")
    return True

def check_0_qa17_lanczos():
    print("=== [4/4] 0-QA17 LANCZOS 平滑放大與 NN 否證 ===")
    variants = ["paint_fox_orange", "paint_ivory_stock", "paint_emerald_glaze"]
    for v in variants:
        p512 = f"{CHASSIS_DIR}/{v}_512.png"
        p128 = f"{CHASSIS_DIR}/{v}.png"
...
def main():
    print("==========================================================")
    print("  狐族底盤切片去頭與去耳 (t_21ece332) 驗收稽核")
    print("==========================================================\n")
    ...
    print("==========================================================")
    print("  🎉 全部驗證全綠通過！(0-ART27, 0-ART26b, 0-QA18, 0-QA17)")
    print("==========================================================")
