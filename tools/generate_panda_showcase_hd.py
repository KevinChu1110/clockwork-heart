...
def process_panda_showcase():
    src_path = "branding/char_panda.png"
    dst_path = "game/assets/sprites/player/showcase/panda_idle_hd.png"
    ...
    print(f"Processing panda ({w}x{h}): bg_mean={bg_mean.astype(int).tolist()}")
    ...
    # Check for between-legs / armpit bg holes (matching bg_mean within tolerance)
    dist_bg = np.abs(arr - bg_mean).max(axis=-1)
    hole_candidates = (~is_bg) & (dist_bg <= 8)
                ...
                while cq:
                    ...
                    for dy, dx in [(-1,0),(1,0),(0,-1),(0,1)]:
                        ...
                        if 0 <= ny < h and 0 <= nx < w and hole_candidates[ny, nx] and not visited[ny, nx]:
                            ...
                            cq.append((ny, nx))
                if len(comp) > 300:
                    print(f"  Found interior bg hole: size={len(comp)}, y range=[{min(p[0] for p in comp)}, {max(p[0] for p in comp)}]")
                    for cy, cx in comp:
                        is_bg[cy, cx] = True
