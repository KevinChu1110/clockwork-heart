extends SceneTree
## 《發條之心》紙娃娃豬族場景節點 (PaperdollCharacter - boar) 無頭自動化測試
## 執行方式：godot --path game --headless -s ../tools/test_paperdoll_boar_scene.gd

const PaperdollRenderer = preload("res://scripts/art/paperdoll_renderer.gd")
const PaperdollCharacter = preload("res://scripts/art/paperdoll_character.gd")
const PROOF_OUTPUT_PATH := "res://assets/sprites/player/paperdoll/boar/proof_paperdoll_boar_composite.png"


func _initialize() -> void:
	var ok := true
	print("=== 開始豬族紙娃娃實際場景節點測試 (PaperdollCharacter - boar) ===")
	...
	if composite_img == null or composite_img.is_empty():
		...
		# 斷言：合成圖絕非空白或全透明（完整 7 層豬族角色應有超過 1000 像素）
	...
	if restored_fox_staff == null or restored_fox_staff.texture == null or not restored_fox_staff.visible:
		...
		print("  ✓ 斷言通過：切回 fox 後所有槽位正常恢復可見與貼圖")
	...
	if restored_boar_hammer == null or restored_boar_hammer.texture == null or not restored_boar_hammer.visible:
		...
		print("  ✓ 斷言通過：切回 boar 後所有槽位正常恢復可見與貼圖")
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("BOAR_PAPERDOLL_SCENE_OK")
		quit(0)
	else:
		print("BOAR_PAPERDOLL_SCENE_FAIL")
		quit(1)
