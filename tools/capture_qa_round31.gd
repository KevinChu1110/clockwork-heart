extends SceneTree
## QA Round 31: 瓷韻熊貓（第十三族）全流程（骨架/切片/官方資產/創角/大廳/衣櫥/探索/戰鬥）探索性 QA 驗收截圖腳本
## 涵蓋：
## 1. 創角畫面 (PaperdollSelectDemo)：瓷韻熊貓預設外裝【禪道學徒生漆長袍】與塗裝【羊脂白瓷生漆塗裝】
## 2. 創角畫面：切換外裝【無外裝 (裸機素體)】，驗證 512 舞台即時更新與 confirm_selection 寫入 GameState
## 3. 衣櫥 (WardrobeDialog) 篩選「全部」驗證 13 族完整清單（含「貓」chip）
## 4. 衣櫥篩選「貓」，選戴【禪道學徒生漆長袍】與【羊脂白瓷生漆塗裝】，即時預覽與選中態卡片
## 5. 衣櫥換裝切換為【無外裝 (裸機素體)】，驗證跨族無污染
## 6. 手遊大廳 (MobileLobby) 驗證瓷韻熊貓 512 高清合成與六語系 (zh_TW, zh_CN, en, ja, ko, es) 即時刷新
## 7. 大廳連動商城即時刷新 (en 英文環境，0-QA25 檢查)
## 8. 探索場景 (ExploreView) 瓷韻熊貓在地圖中行走/待機，驗證不退回 128 糊圖
## 9. 戰鬥畫面 (BattleView) 瓷韻熊貓出戰對戰狼，PlayerBody 為 512x512 高清合成
## 10. 戰鬥名稱備援對照（空名 fallback 瓷韻熊貓族名與暴擊打擊事件戰鬥日誌）
## 11. 瓷韻熊貓 7 槽切片與骨架驗證 (DevPaperdollPreview)
## 12. 特寫 Crops 產出

const OUT_DIR := "/opt/side/bravesoul-game/proofs/qa_round31"
const CROPS_DIR := "/opt/side/bravesoul-game/proofs/qa_round31/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行 QA Round 31 瓷韻熊貓全流程探索性實機截圖 ──")
	_step = 1
	_wait = 0
...
func _process(_delta: float) -> bool:
	...
	match _step:
		1:
			# 步驟 1: 創角介面 (PaperdollSelectDemo) 選中瓷韻熊貓預設外裝與塗裝
			if _wait == 1:
				if _loc_node:
				...
				var path := "%s/proof_01_creation_panda_default.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [1/16] 創角選用瓷韻熊貓預設外裝【禪道學徒生漆長袍】與【羊脂白瓷生漆塗裝】: %s" % path)
		...
		2:
			...
			if _wait == 5:
				if _current_node:
					var data: Dictionary = _current_node.get("RACES_DATA").get("panda", {})
				...
				var path := "%s/proof_02_creation_panda_bare.png" % OUT_DIR
				_save_screenshot(path)
		...
		3:
			...
			if _wait == 1:
				...
				_save_screenshot(path)
				print("  ✓ [3/16] 衣櫥篩選「全部」驗證 13 族完整清單: %s" % path)
				_step = 4
				_wait = 0

		4:
			# 步驟 4: 衣櫥切換為「貓」篩選，選戴【禪道學徒生漆長袍】與【羊脂白瓷生漆塗裝】
			if _wait == 5:
				if _current_node and _current_node.has_method("set_race_filter"):
				...
				var path := "%s/proof_04_wardrobe_panda_robe_equipped.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [4/16] 衣櫥選用【禪道學徒生漆長袍】與【羊脂白瓷生漆塗裝】: %s" % path)
				_step = 5
				_wait = 0

		5:
			# 步驟 5: 衣櫥切換為【無外裝 (裸機素體)】對照，並驗證無污染
			if _wait == 5:
				var scroll: ScrollContainer = _current_node.find_child("FilterScroll", true, false) as ScrollContainer
				if scroll:
					var hbar := scroll.get_h_scroll_bar()
				...
				var path := "%s/proof_05_wardrobe_panda_bare_equipped.png" % OUT_DIR
				_save_screenshot(path)
		...
		6:
			# 步驟 6: 建立手遊大廳 (MobileLobby)，套用瓷韻熊貓全套配裝
			if _wait == 1:
				if _gs:
					_gs.call("reset_new_game", "panda")
				...
				if _current_node and _current_node.has_method("_hero_display_tex"):
					...
					print("    大廳瓷韻熊貓英雄展示貼圖寬度: %d (>= 256 為 512 高清合成，嚴格遵守 lobby-no-128)" % w)
				_step = 7
				_wait = 0
		...
		7, 8, 9, 10, 11, 12:
			...
			if _wait == 1:
				...
				var path := "%s/proof_06_lobby_panda_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/16] 已截取大廳瓷韻熊貓 [%s] 語系實機截圖: %s" % [loc_idx + 6, code, path])
		...
		14:
			# 步驟 14: 探索場景 (ExploreView) 瓷韻熊貓在地圖中行走/待機驗證
			if _wait == 1:
				if _gs:
					_gs.call("reset_new_game", "panda")
					_gs.set("player_race", "panda")
				...
				var path := "%s/proof_08_explore_panda.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [13/16] 已截取探索場景瓷韻熊貓展示: %s" % path)
				if _current_node:
		...
		15:
			# 步驟 15: 戰鬥畫面 (BattleView) 瓷韻熊貓穿戴專屬裝備出戰
			if _wait == 1:
				if _gs:
					_gs.call("reset_new_game", "panda")
					_gs.set("player_race", "panda")
				...
				var path := "%s/proof_09_battle_panda_combat.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [14/16] 已截取戰鬥畫面瓷韻熊貓出戰: %s" % path)
		...
		17:
			# 步驟 17: 瓷韻熊貓（第十三族 panda）骨架與 7 大槽位切片驗證 (DevPaperdollPreview)
			if _wait == 1:
				...
				var path := "%s/proof_11_panda_paperdoll_slices_verification.png" % OUT_DIR
				_save_screenshot(path)
				print("  ✓ [16/16] 已截取瓷韻熊貓骨架與 7 大槽位切片驗證: %s" % path)
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 18
...
func _generate_crops() -> void:
	...
	# 7. 瓷韻熊貓紙娃娃切片與骨架特寫
	var panda_p := "%s/proof_11_panda_paperdoll_slices_verification.png" % OUT_DIR
	if FileAccess.file_exists(panda_p):
		var img := Image.load_from_file(panda_p)
		if img and not img.is_empty():
			var crop := img.get_region(Rect2i(420, 140, 440, 420))
			var crop_p := "%s/crop_panda_paperdoll_stage.png" % CROPS_DIR
			crop.save_png(crop_p)
			print("  ✓ 已產出特寫: %s" % crop_p)
