extends SceneTree
## 《發條之心》紙娃娃獅族場景節點 (PaperdollCharacter) 無頭自動化測試
## 執行方式：godot --path game --headless -s ../tools/test_paperdoll_lion_scene.gd

const PaperdollRenderer = preload("res://scripts/art/paperdoll_renderer.gd")
const PaperdollCharacter = preload("res://scripts/art/paperdoll_character.gd")
const PROOF_OUTPUT_PATH := "res://assets/sprites/player/paperdoll/lion/proof_paperdoll_lion_composite.png"


func _initialize() -> void:
	var ok := true
	print("=== 開始獅族紙娃娃實際場景節點測試 (PaperdollCharacter - lion) ===")
	...
	if composite_img == null or composite_img.is_empty():
		...
		# 斷言：合成圖絕非空白或全透明（完整 7 層獅族角色應有超過 1000 像素）
	...
	if restored_lion_lance == null or restored_lion_lance.texture == null or not restored_lion_lance.visible:
		...
		print("  ✓ 斷言通過：切回 lion 後所有槽位正常恢復可見與貼圖")
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("LION_PAPERDOLL_SCENE_OK")
		quit(0)
	else:
		print("LION_PAPERDOLL_SCENE_FAIL")
		quit(1)
