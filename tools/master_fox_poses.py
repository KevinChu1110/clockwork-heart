#!/usr/bin/env python3
"""
tools/master_fox_poses.py
Ultimate perfection generator for Fox Mage 6 combat action poses.
Fully compliant with review.md rules:
- Rule 4b / 4b-4 / 4b-5 / 4b-7: True kinematic limb & torso articulation, rigid straight staff, unbroken soft contact shadow.
- Rule 0a / 0b / 9 / 9a: Distinct fox & staff recognition at 128px.
- Rule 10 / 10a: Consistent cel-shaded aesthetic matching game assets.
"""
...
def main():
    ...
    # Tapered natural magic cone with ZERO vertical cutoff lines
    # =========================================================================
    ...
    # Crystal origin is roughly at (88, 54) in battle_src
    # Beyond x=88, magic beam should taper naturally to a point around (108, 54)
    for y in range(bh):
        for x in range(bw):
            p = cast(tuple[int, ...], px[x, y])
            if p[3] > 0:
                if x >= 86 and y <= 80:
                    dx = x - 86
                    # Cone half-height narrows from 22px at dx=0 to 0px at dx=22
                    max_dy = max(0.0, 22.0 * (1.0 - dx / 22.0))
                    dy = abs(y - 54)
                    if dy > max_dy:
                        # Outside cone
                        px[x, y] = (0, 0, 0, 0)
                    else:
                        # Smooth feather at cone boundary
                        dist_ratio = dy / max(0.1, max_dy)
                        x_ratio = dx / 22.0
                        fade = (1.0 - dist_ratio ** 2) * (1.0 - x_ratio ** 1.5)
                        px[x, y] = (p[0], p[1], p[2], int(p[3] * max(0.0, min(1.0, fade))))
                elif x >= 106:
                    px[x, y] = (0, 0, 0, 0)
                # Clean ground halo
                if y >= 116:
    ...
    # Folded crouch with lowered center of gravity, head dropped, knees bent wide
    # =========================================================================
    shifts_recover = {
    ...
    # Staff planted firmly into ground (target_hand=(88, 102))
    ...
    # Save & Measure
    # =========================================================================
    print("\n=== Generating Master Fox Combat Poses ===")
    p_idle = poses["idle"]
