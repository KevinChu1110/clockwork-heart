#!/usr/bin/env python3
"""
verify_tortoise_poses.py
Comprehensive verification test suite for Xuanji Tortoise combat action poses (128x128 RGBA & 512x512 RGBA).
Mirrors verify_crane_poses.py, verify_tiger_poses.py, and verify_bear_poses.py.
"""
import os
import hashlib
...
POSES_DIR = f"{REPO_ROOT}/game/assets/sprites/player/poses/tortoise"
EXPECTED_SHADOW = [57, 35, 0, 0, 0, 0, 0, 0, 0, 0]
POSES = ['idle', 'attack', 'hit', 'recover', 'skill', 'telegraph']
lut = [0] * 11 + [1] * 245

print("=== VERIFYING XUANJI TORTOISE COMBAT POSES (128x128 & 512x512 RGBA) ===")
...
for p in POSES:
    ...
    assert diff_pct <= 6.0, f"FAIL: {p} height diff {diff_pct:.2f}% > 6%!"

print("\n=== ALL AUTOMATED METRIC VERIFICATIONS COMPLETED SUCCESSFULLY ===")
