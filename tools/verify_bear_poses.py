#!/usr/bin/env python3
"""
verify_bear_poses.py
Comprehensive verification test suite for The Iron Bear (玄軸熊) combat action poses (128x128 RGBA).
Mirrors verify_tiger_poses.py, verify_lion_poses.py, and validate_final_6_poses.py.
"""
import os
import hashlib
...
POSES_DIR = f"{REPO_ROOT}/game/assets/sprites/player/poses/bear"
BATTLE_DIR = f"{REPO_ROOT}/game/assets/sprites/player/battle/bear"
POSES = ['idle', 'attack', 'hit', 'recover', 'skill', 'telegraph']
lut = [0] * 11 + [1] * 245

print("=== VERIFYING THE IRON BEAR COMBAT POSES (128x128 RGBA) ===")
...
for p in POSES:
    ...
    bad_bounds = []
    for x in range(128):
        if px[x, 0][3] > 0: bad_bounds.append(f"top row y=0 at x={x}")
        if px[x, 1][3] > 0: bad_bounds.append(f"top row y=1 at x={x}")
        if px[x, 126][3] > 0: bad_bounds.append(f"bot row y=126 at x={x}")
        if px[x, 127][3] > 0: bad_bounds.append(f"bot row y=127 at x={x}")
    for y in range(128):
        if px[0, y][3] > 0: bad_bounds.append(f"left col x=0 at y={y}")
        if px[1, y][3] > 0: bad_bounds.append(f"left col x=1 at y={y}")
        if px[124, y][3] > 0: bad_bounds.append(f"right col x=124 at y={y}")
        if px[125, y][3] > 0: bad_bounds.append(f"right col x=125 at y={y}")
        if px[126, y][3] > 0: bad_bounds.append(f"right col x=126 at y={y}")
        if px[127, y][3] > 0: bad_bounds.append(f"right col x=127 at y={y}")
    if bad_bounds:
...
idle_shadow = [sum(1 for x in range(128) if images['idle'].getpixel((x, y))[3] > 20) for y in range(118, 128)]
print(f"Benchmark Bear shadow counts: {idle_shadow}")
shadow_ok = True
for p in POSES:
    ...
    if counts != idle_shadow:
        ...
        shadow_ok = False
assert shadow_ok, f"FAIL: Ground shadow check failed on {p}"
...
idle_h = images['idle'].getbbox()[3] - images['idle'].getbbox()[1]
for p in POSES:
    bbox = images[p].getbbox()
    ...
    # In skill pose, overhead hammer is raised, but body height is preserved
    assert diff_pct <= 5.5, f"FAIL: {p} height diff {diff_pct:.2f}% > 5.5%"

# Check body pixel volume (y < 115)
idle_vol = sum(1 for y in range(115) for x in range(128) if images['idle'].getpixel((x, y))[3] > 8)
print(f"\n--- Body Pixel Volume Verification (y<115, idle={idle_vol} px) ---")
for p in POSES:
    vol = sum(1 for y in range(115) for x in range(128) if images[p].getpixel((x, y))[3] > 8)
