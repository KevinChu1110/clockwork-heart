...
Criteria: Color distance between head dominant color and chassis dominant color must be < 60.0.
"""
...
def main():
    ...
    pairs = [
        ...
        ("Bear", "Ivory Stock", 
         ...
         f"{REPO}/game/assets/sprites/player/paperdoll/bear/chassis/paint_ivory_stock_512.png"),
        ("Bear", "Amber", 
         f"{REPO}/game/assets/sprites/player/paperdoll/bear/head_unit/head_iron_bear_amber_512.png",
         f"{REPO}/game/assets/sprites/player/paperdoll/bear/chassis/paint_bear_amber_512.png"),
        ("Bear", "Quarry", 
         f"{REPO}/game/assets/sprites/player/paperdoll/bear/head_unit/head_iron_bear_quarry_512.png",
         f"{REPO}/game/assets/sprites/player/paperdoll/bear/chassis/paint_iron_quarry_512.png"),
        
        # Penguin
        ("Penguin", "Navy Stock", 
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/head_unit/head_steam_penguin_stock_512.png",
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/chassis/paint_penguin_navy_512.png"),
        ("Penguin", "Ivory", 
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/head_unit/head_steam_penguin_ivory_512.png",
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/chassis/paint_ivory_stock_512.png"),
        ("Penguin", "Polar Frost", 
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/head_unit/head_steam_penguin_polar_512.png",
         f"{REPO}/game/assets/sprites/player/paperdoll/penguin/chassis/paint_polar_frost_512.png"),
    ]

    all_pass = True
    print(f"{'族系':8s} | {'塗裝':12s} | {'Head 主色 (RGB)':20s} | {'Chassis 主色 (RGB)':20s} | {'色距':6s} | {'判定'}")
    print("-" * 88)

    for race, variant, head_p, ch_p in pairs:
        ...
        if dist >= 60.0:
            all_pass = False
        print(f"{race:8s} | {variant:12s} | {str(c_head):20s} | {str(c_ch):20s} | {dist:6.1f} | {status}")

    print("-" * 88)
    if all_pass:
        print("🎉 全部 6 組變體頭部與身體主色色距均小於 60.0，驗收通過！")
    else:
