...
static func _village() -> Dictionary:
	...
	m["entities"] = [
		...
		_e("maisui", 1780, 960, 48, 64, "舊鑰", Color(0.85, 0.55, 0.45)),
		_e("well", 1820, 820, 52, 52, "八音盒", Color(0.4, 0.42, 0.45), true),
		_e("sword", 880, 960, 48, 48, "鏽蝕長劍", Color(0.55, 0.5, 0.4)),
		...
		_e("exit_east", 2460, 840, 72, 80, "往玩具堆外緣", Color(0.3, 0.25, 0.2)),
		...
		_e("message_stone", 1680, 1060, 48, 52, "泛黃便簽", Color(0.5, 0.55, 0.7), true),
	]
	return m
...
static func _village_outskirts() -> Dictionary:
	...
	m["entities"] = [
		...
		_e("scare_field", 653, 1040, 48, 56, "棄置哨兵偶", Color(0.4, 0.35, 0.25)),
		_e("pond", 1040, 867, 80, 48, "積水碟盤", Color(0.3, 0.35, 0.4), true),
		_e("woodpile", 870, 547, 56, 40, "拆開的積木條", Color(0.45, 0.35, 0.25)),
		_e("to_mill", 1425, 233, 64, 64, "發條風車台", Color(0.4, 0.42, 0.35)),
		_e("trail_east", 2292, 669, 72, 64, "接玩具堆外緣", Color(0.35, 0.4, 0.3)),
		_e("windmill", 1726, 236, 60, 80, "停擺發條風車", Color(0.4, 0.38, 0.35), true),
...
static func _road() -> Dictionary:
	var m := _base("玩具堆外緣 · 朝向世界大鐘", Color(0.12, 0.16, 0.22), 2048, 1152, Vector2(1451, 1045), "road")
	m["entities"] = [
		_e("look_back", 1359, 972, 48, 48, "停擺的閣樓", Color(0.4, 0.35, 0.35)),
		_e("milepost", 1278, 815, 40, 56, "里程碑·一", Color(0.45, 0.42, 0.4), true),
		_e("to_road_inn", 1068, 566, 64, 56, "停擺旅舍", Color(0.5, 0.4, 0.3)),
		_e("bush_a", 867, 570, 48, 40, "灌木", Color(0.3, 0.35, 0.28)),
		_e("wolf", 782, 409, 64, 64, "齒輪異響", Color(0.35, 0.3, 0.35)),
		_e("milepost_b", 1033, 380, 40, 56, "里程碑·二", Color(0.45, 0.42, 0.4), true),
		_e("to_road_bridge", 393, 332, 64, 56, "大橋段", Color(0.4, 0.42, 0.45)),
		_e("to_road_ruins", 1032, 185, 64, 56, "古驛廢墟", Color(0.4, 0.38, 0.35)),
		_e("bush_b", 1133, 149, 48, 40, "發條雜叢", Color(0.28, 0.3, 0.28)),
		_e("road_stone", 971, 259, 40, 36, "裂開的路石", Color(0.4, 0.38, 0.36)),
		_e("camp_ash", 620, 414, 56, 40, "過客熄火堆", Color(0.4, 0.3, 0.25)),
		_e("bridge", 512, 381, 80, 48, "小橋", Color(0.4, 0.38, 0.35), true),
		_e("dawn_glow", 1282, 208, 56, 56, "遠方鐘影", Color(0.55, 0.5, 0.4)),
		_e("exit_town_hint", 104, 330, 72, 64, "通往世界大鐘方向", Color(0.4, 0.4, 0.45)),
	]
	return m
...
static func _town_soul() -> Dictionary:
	...
	m["entities"] = [
		...
		_e("astrolabe", 508, 508, 56, 48, "封靈台", Color(0.4, 0.45, 0.7), true),
...
static func _wild() -> Dictionary:
	...
	var ents: Array = [
		...
		_e("burnt_field", 320, 937, 64, 48, "焦黑積木田邊", Color(0.45, 0.38, 0.2)),
		_e("camp", 576, 1073, 64, 52, "焦黑積木田", Color(0.45, 0.4, 0.2), true),
		_e("scarecrow", 768, 781, 40, 56, "焦黑哨兵偶", Color(0.4, 0.35, 0.25)),
...
static func _crossroads() -> Dictionary:
	var m := _base("六域岔路 · 沙盤之心道", Color(0.1, 0.12, 0.1), 2048, 1152, Vector2(934, 769), "road")
	var ents: Array = [
		_e("sign_board", 965, 438, 64, 56, "六域路標", Color(0.45, 0.4, 0.3), true),
		_e("camp_fire", 783, 655, 48, 48, "旅人營火", Color(0.9, 0.4, 0.15), true),
		_e("merchant", 845, 686, 48, 64, "行商", Color(0.6, 0.5, 0.4)),
		_e("path_knight", 124, 453, 72, 64, "西·堡壘", Color(0.5, 0.45, 0.4)),
		_e("path_mist_c", 1435, 1044, 72, 64, "南·白霧之地", Color(0.5, 0.55, 0.7)),
		_e("path_dojo_c", 1189, 100, 72, 64, "北·道場", Color(0.4, 0.55, 0.35)),
		_e("path_forest_c", 1640, 100, 72, 64, "東北·森林", Color(0.35, 0.55, 0.4)),
		_e("path_coast_c", 1640, 375, 72, 64, "東南·海岸", Color(0.4, 0.55, 0.65)),
		_e("path_tower_c", 1844, 141, 72, 64, "東·通天塔", Color(0.45, 0.35, 0.55)),
		_e("to_cross_north", 1108, 108, 64, 56, "北山道", Color(0.4, 0.5, 0.4)),
		_e("to_cross_east", 1928, 297, 64, 56, "東塔荒原道", Color(0.4, 0.35, 0.45)),
		_e("to_caravan", 617, 530, 64, 56, "行商驛站", Color(0.55, 0.45, 0.35)),
		_e("to_starfall", 412, 220, 64, 56, "星落平原", Color(0.45, 0.5, 0.7)),
		_e("to_hunt", 1231, 687, 64, 56, "野外獵場", Color(0.65, 0.35, 0.4)),
		_e("ruin_pillar", 1071, 281, 40, 64, "古柱", Color(0.4, 0.38, 0.36), true),
		_e("save_cross", 865, 843, 48, 48, "存檔石", Color(0.4, 0.45, 0.5)),
		_e("world_map_stone", 1130, 563, 56, 48, "世界輿圖石", Color(0.4, 0.5, 0.55)),
	]
	## 雷歐後、未完結前：岔路擋道的黑鏽浪人
	if _flag("boss.leo_cleared") and not _flag("side.ronin_done"):
		ents.append(_e("ronin", 1028, 654, 52, 68, "黑鏽浪人", Color(0.35, 0.2, 0.35)))
	elif _flag("side.ronin_spared"):
		ents.append(_e("ronin", 1029, 655, 48, 64, "浪人（收刃）", Color(0.4, 0.35, 0.4)))
	m["entities"] = ents
	return m
...
static func _hunting_grounds() -> Dictionary:
	...
	m["entities"] = [
		...
		_e("camp_ash", 612, 1178, 56, 40, "獵手熄火堆", Color(0.4, 0.3, 0.25)),
...
static func _mist() -> Dictionary:
	var m := _base("白霧之地 · 外圍", Color(0.1, 0.11, 0.16), 3000, 1674, Vector2(220, 850), "mist_village")
	var ents: Array = [
		_e("fog_hide", 436, 953, 48, 64, "白霧之地", Color(0.5, 0.55, 0.7)),
...
static func _mist_deep() -> Dictionary:
	var m := _base("白霧之地 · 深巷與影廊前", Color(0.08, 0.09, 0.14), 2200, 1228, Vector2(348, 841), "mist_village")
	m["entities"] = [
		_e("back_mist", 320, 761, 56, 56, "回白霧之地", Color(0.45, 0.5, 0.6)),
...
static func _mist_cliff() -> Dictionary:
	var m := _base("白霧之地 · 霧崖觀台", Color(0.09, 0.1, 0.15), 2200, 1238, Vector2(1210, 1176), "mist_cliff")
	m["entities"] = [
		_e("back_from_mist_sub", 1144, 1114, 56, 56, "回白霧之地", Color(0.45, 0.5, 0.6)),
...
static func _mist_shrine() -> Dictionary:
	var m := _base("白霧之地 · 霧祠內殿", Color(0.08, 0.09, 0.14), 2000, 1116, Vector2(640, 1107), "mist_shrine")
...
static func _mist_mirror() -> Dictionary:
	var m := _base("白霧之地 · 鏡廊迷宮", Color(0.07, 0.08, 0.13), 2400, 1350, Vector2(232, 1295), "mist_mirror")
	m["entities"] = [
...
static func _dojo() -> Dictionary:
	...
	var ents: Array = [
		...
		_e("back_mist", 180, 1407, 56, 48, "回白霧之地", Color(0.45, 0.5, 0.6)),
...
static func _coast_wreck() -> Dictionary:
	...
	m["entities"] = [
		...
		_e("mast", 1016, 100, 40, 80, "斷桅", Color(0.42, 0.38, 0.32), true),
