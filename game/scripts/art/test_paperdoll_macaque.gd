...
func _init() -> void:
	print("=== RUNNING MACAQUE PAPERDOLL TEST (PaperdollRenderer) ===")
	...
	var path_map := PaperdollRenderer.build_paperdoll_map("macaque")
	print("Slot paths resolved for macaque:")
	var failed := false
	for slot_def in slots:
		...
		if not path.begins_with("res://assets/sprites/player/paperdoll/macaque/"):
			printerr("  ❌ ERROR: Slot %s did not resolve to macaque paperdoll slice: %s" % [sid, path])
			failed = true
	...
	if failed:
		...
		print("=== ALL 7 MACAQUE PAPERDOLL SLOTS VERIFIED PERFECTLY (128x128) ===")
		print("PAPERDOLL_MACAQUE_OK")
		quit(0)
