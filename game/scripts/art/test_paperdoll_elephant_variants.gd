extends SceneTree
## 《發條之心》鋼岳象族（第十一族）第二套外裝/塗裝變體 (PaperdollCharacter - Elephant Variants) 無頭驗證測試
## 執行方式：godot --path game --headless -s res://scripts/art/test_paperdoll_elephant_variants.gd
...
func _initialize() -> void:
	var ok := true
	print("=== 開始鋼岳象族（第十一族）第二套塗裝/外裝變體無頭驗證 (Elephant Variants Test) ===")
	...
	# 1. 測試新外裝 costume_colossus_bastion_plate
	print("\n--- 1. 驗證新外裝 costume_colossus_bastion_plate ---")
	character.render_character("elephant", {"costume": "costume_colossus_bastion_plate"})
	var c_sprite: Sprite2D = character.get_slot_sprite("costume")
	if c_sprite == null or c_sprite.texture == null:
		push_error("costume_colossus_bastion_plate 貼圖載入失敗！")
		ok = false
	else:
		var sz := c_sprite.texture.get_size()
		print("  ✓ costume_colossus_bastion_plate 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(c_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("costume_colossus_bastion_plate 尺寸不為 128x128")
			ok = false

	# 2. 測試新塗裝 paint_tungsten_iron
	print("\n--- 2. 驗證新塗裝 paint_tungsten_iron ---")
	character.render_character("elephant", {"chassis": "paint_tungsten_iron"})
	var ch_sprite: Sprite2D = character.get_slot_sprite("chassis")
	if ch_sprite == null or ch_sprite.texture == null:
		push_error("paint_tungsten_iron 貼圖載入失敗！")
		ok = false
	else:
		var sz := ch_sprite.texture.get_size()
		print("  ✓ paint_tungsten_iron 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(ch_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("paint_tungsten_iron 尺寸不為 128x128")
			ok = false

	# 3. 測試雙新變體同時混搭合成 (tungsten + bastion)
	print("\n--- 3. 驗證雙新變體同時混搭合成 (tungsten + bastion) ---")
	character.render_character("elephant", {
	...
	character.render_character("elephant", {"costume": "none"})
	...
	# 5. 測試原廠黃銅塗裝 + 新外裝混搭 (brass + bastion)
	print("\n--- 5. 驗證原廠黃銅塗裝 + 新外裝混搭 (brass + bastion) ---")
	character.render_character("elephant", {
	...
	# 6. 測試新塗裝 + 原廠外裝混搭 (tungsten + overalls)
	print("\n--- 6. 驗證新塗裝 + 原廠外裝混搭 (tungsten + overalls) ---")
	character.render_character("elephant", {
		"chassis": "paint_tungsten_iron",
		...
		for y in range(comp_img3.get_height()):
			for x in range(comp_img3.get_width()):
				if comp_img3.get_pixel(x, y).a > 0.05:
					non_trans3 += 1
		print("  ✓ 新塗裝 + 原廠外裝混搭合成非透明像素數: %d 像素" % non_trans3)
		if non_trans3 < 1000:
			push_error("非透明像素過少！")
			ok = false
...
func _finish(success: bool) -> void:
	print("\n=======================================================")
	if success:
		print("ELEPHANT_VARIANTS_TEST_OK")
		print("🎉 [ALL PASS] 鋼岳象第二套外裝與塗裝變體紙娃娃無頭驗證全數通過！")
		quit(0)
	else:
		printerr("❌ [FAIL] 鋼岳象第二套外裝與塗裝變體紙娃娃驗證失敗！")
		quit(1)
