extends SceneTree
## 《發條之心》玄機龜族第二套外裝/塗裝變體 (PaperdollCharacter - Tortoise Variants) 無頭驗證測試
## 執行方式：godot --path game --headless -s res://scripts/art/test_paperdoll_tortoise_variants.gd
...
func _initialize() -> void:
	var ok := true
	print("=== 開始玄機龜族第二套塗裝/外裝變體無頭驗證 (Tortoise Variants Test) ===")
	...
	# 1. 測試新外裝 costume_bagua_master_robe
	print("\n--- 1. 驗證新外裝 costume_bagua_master_robe ---")
	character.render_character("tortoise", {"costume": "costume_bagua_master_robe"})
	var c_sprite: Sprite2D = character.get_slot_sprite("costume")
	if c_sprite == null or c_sprite.texture == null:
		push_error("costume_bagua_master_robe 貼圖載入失敗！")
		ok = false
	else:
		var sz := c_sprite.texture.get_size()
		print("  ✓ costume_bagua_master_robe 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(c_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("costume_bagua_master_robe 尺寸不為 128x128")
			ok = false

	# 2. 測試新塗裝 paint_basalt_black
	print("\n--- 2. 驗證新塗裝 paint_basalt_black ---")
	character.render_character("tortoise", {"chassis": "paint_basalt_black"})
	var ch_sprite: Sprite2D = character.get_slot_sprite("chassis")
	if ch_sprite == null or ch_sprite.texture == null:
		push_error("paint_basalt_black 貼圖載入失敗！")
		ok = false
	else:
		var sz := ch_sprite.texture.get_size()
		print("  ✓ paint_basalt_black 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(ch_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("paint_basalt_black 尺寸不為 128x128")
			ok = false

	# 3. 測試雙新變體同時混搭合成 (basalt + bagua)
	print("\n--- 3. 驗證雙新變體同時混搭合成 (basalt + bagua) ---")
	character.render_character("tortoise", {
	...
	character.render_character("tortoise", {"costume": "none"})
	...
	# 5. 測試原廠青銅塗裝 + 新外裝混搭 (jade + bagua)
	print("\n--- 5. 驗證原廠青銅塗裝 + 新外裝混搭 (jade + bagua) ---")
	character.render_character("tortoise", {
	...
	# 6. 測試新塗裝 + 原廠外裝混搭 (basalt + harness)
	print("\n--- 6. 驗證新塗裝 + 原廠外裝混搭 (basalt + harness) ---")
	character.render_character("tortoise", {
		"chassis": "paint_basalt_black",
...
func _finish(success: bool) -> void:
	print("\n=======================================================")
	if success:
		print("TORTOISE_VARIANTS_TEST_OK")
		print("🎉 [ALL PASS] 玄機龜第二套外裝與塗裝變體紙娃娃無頭驗證全數通過！")
		quit(0)
	else:
		print("TORTOISE_VARIANTS_TEST_FAIL")
		push_error("\n❌ [FAIL] 玄機龜第二套外裝與塗裝變體紙娃娃驗證失敗！")
		quit(1)
