extends SceneTree
## 《發條之心》兔族第二套外裝/塗裝變體 (PaperdollCharacter - Rabbit Variants) 無頭驗證測試
## 執行方式：godot --path game --headless -s res://scripts/art/test_paperdoll_rabbit_variants.gd
...
func _initialize() -> void:
	var ok := true
	print("=== 開始兔族第二套塗裝/外裝變體無頭驗證 (Rabbit Variants Test) ===")
	...
	# 1. 測試新外裝 costume_royal_parade
	print("\n--- 1. 驗證新外裝 costume_royal_parade ---")
	character.render_character("rabbit", {"costume": "costume_royal_parade"})
	var c_sprite: Sprite2D = character.get_slot_sprite("costume")
	if c_sprite == null or c_sprite.texture == null:
		push_error("costume_royal_parade 貼圖載入失敗！")
		ok = false
	else:
		var sz := c_sprite.texture.get_size()
		print("  ✓ costume_royal_parade 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(c_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("costume_royal_parade 尺寸不為 128x128")
			ok = false

	# 2. 測試新塗裝 paint_midnight_navy
	print("\n--- 2. 驗證新塗裝 paint_midnight_navy ---")
	character.render_character("rabbit", {"chassis": "paint_midnight_navy"})
	var ch_sprite: Sprite2D = character.get_slot_sprite("chassis")
	if ch_sprite == null or ch_sprite.texture == null:
		push_error("paint_midnight_navy 貼圖載入失敗！")
		ok = false
	else:
		var sz := ch_sprite.texture.get_size()
		print("  ✓ paint_midnight_navy 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(ch_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("paint_midnight_navy 尺寸不為 128x128")
			ok = false

	# 3. 測試雙新變體同時混搭合成 (midnight_navy + royal_parade)
	print("\n--- 3. 驗證雙新變體同時混搭合成 (midnight_navy + royal_parade) ---")
	...
	character.render_character("rabbit", {"costume": "none"})
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("RABBIT_VARIANTS_TEST_OK")
		quit(0)
	else:
		print("RABBIT_VARIANTS_TEST_FAIL")
		quit(1)
