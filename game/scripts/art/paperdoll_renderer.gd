...
static func resolve_slot_texture_path(race: String, slot_id: String, item_id: String = "") -> String:
	...
	if effective_id != "":
		...
		# 1b. 外裝／武器／鑰匙／奇玩／塗裝跨種族共用：本族沒有切片就找其他族同檔名（僅限十三大正式族系）
	...
	match slot_id:
		SLOT_CHASSIS:
			if race == "fox":
				...
				return "paint_frog_emerald"
			elif race == "panda":
				return "paint_panda_porcelain"
			return "paint_ivory_stock"
		SLOT_HEAD_UNIT:
			if race == "macaque":
				...
				return "head_spring_frog_stock"
			elif race == "panda":
				return "head_panda_brass_socket_ears"
		...
		SLOT_WINDING_KEY:
			if race == "tiger":
				...
				return "key_twin_wing_concentric"
			elif race == "panda":
				return "key_panda_taiji_ruyi_brass"
			elif race == "fox":
				return ""
		...
		SLOT_COSTUME:
			if race == "macaque":
				...
				return "costume_spring_forest_courier"
			elif race == "panda":
				return "costume_panda_zen_apprentice_robe"
			return "costume_nutcracker_guard"
		SLOT_OPTIC_CORE:
			if race == "lion":
				...
				return "core_azure_aperture"
			elif race == "panda":
				return "core_obsidian_amber_quartz"
			elif race == "boar":
		...
		SLOT_WEAPON:
			if race == "macaque":
				...
				return "wpn_lotus_cog_dart"
			elif race == "panda":
				return "wpn_panda_taiji_cestus"
		...
		SLOT_BACK_CURIO:
			if race == "macaque":
				...
				return "curio_lotus_leaf_parasol"
			elif race == "panda":
				return "curio_panda_floating_taiji_box"
...
static func resolve_slot_texture_path_512(race: String, slot_id: String, item_id: String = "") -> String:
	...
	if effective_id != "":
		...
		if ResourceLoader.exists(p512_clean) or FileAccess.file_exists(p512_clean):
			return p512_clean
		# 1b. 頭部機關塗裝解析分流 (rabbit, bear, penguin)
		if sid == SLOT_HEAD_UNIT:
			if rid == "rabbit":
				if clean_id.find("midnight") >= 0 or clean_id.find("navy") >= 0:
					var p_r := "%s/%s/%s/ear_rabbit_straight_midnight_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_r) or FileAccess.file_exists(p_r): return p_r
				elif clean_id.find("brass") >= 0:
					var p_r := "%s/%s/%s/ear_rabbit_straight_brass_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_r) or FileAccess.file_exists(p_r): return p_r
			elif rid == "bear":
				if clean_id.find("amber") >= 0:
					var p_b := "%s/%s/%s/head_iron_bear_amber_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_b) or FileAccess.file_exists(p_b): return p_b
				elif clean_id.find("quarry") >= 0 or clean_id.find("iron") >= 0:
					var p_b := "%s/%s/%s/head_iron_bear_quarry_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_b) or FileAccess.file_exists(p_b): return p_b
				elif clean_id.find("ivory") >= 0 or clean_id.find("stock") >= 0:
					var p_b := "%s/%s/%s/head_iron_bear_stock_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_b) or FileAccess.file_exists(p_b): return p_b
			elif rid == "penguin":
				if clean_id.find("navy") >= 0 or clean_id.find("penguin_navy") >= 0:
					var p_p := "%s/%s/%s/head_steam_penguin_stock_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_p) or FileAccess.file_exists(p_p): return p_p
				elif clean_id.find("polar") >= 0 or clean_id.find("frost") >= 0:
					var p_p := "%s/%s/%s/head_steam_penguin_polar_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_p) or FileAccess.file_exists(p_p): return p_p
				elif clean_id.find("ivory") >= 0 or clean_id.find("stock") >= 0:
				...
				if clean_id.find("midnight") >= 0 or clean_id.find("navy") >= 0:
					...
					var p_l := "%s/%s/%s/ear_lion_gilded_mane_brass_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_l) or FileAccess.file_exists(p_l): return p_l
				elif clean_id.find("stock") >= 0:
				...
				if clean_id.find("emerald") >= 0 or clean_id.find("glaze") >= 0:
					...
					if ResourceLoader.exists(p_f) or FileAccess.file_exists(p_f): return p_f
				elif clean_id.find("orange") >= 0:
					var p_f := "%s/%s/%s/ear_fox_radar_orange_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_f) or FileAccess.file_exists(p_f): return p_f
				elif clean_id.find("stock") >= 0:
				...
				if clean_id.find("crimson") >= 0 or clean_id.find("molten") >= 0:
					...
					var p_b := "%s/%s/%s/ear_boar_rivet_cowl_ivory_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_b) or FileAccess.file_exists(p_b): return p_b
				elif clean_id.find("stock") >= 0:
				...
				if clean_id.find("bronze") >= 0 or clean_id.find("bamboo") >= 0:
					...
					var p_m := "%s/%s/%s/ear_macaque_coaxial_ivory_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_m) or FileAccess.file_exists(p_m): return p_m
				elif clean_id.find("stock") >= 0:
				...
				if clean_id.find("volcano") >= 0 or clean_id.find("black") >= 0:
					...
					if ResourceLoader.exists(p_t) or FileAccess.file_exists(p_t): return p_t
				elif clean_id.find("ember") >= 0 or clean_id.find("orange") >= 0:
					var p_t := "%s/%s/%s/head_ember_tiger_ember_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_t) or FileAccess.file_exists(p_t): return p_t
				elif clean_id.find("stock") >= 0:
				...
				if clean_id.find("azure") >= 0 or clean_id.find("zephyr") >= 0:
					...
					if ResourceLoader.exists(p_c) or FileAccess.file_exists(p_c): return p_c
				elif clean_id.find("porcelain") >= 0:
					var p_c := "%s/%s/%s/head_cloud_crane_porcelain_512.png" % [PAPERDOLL_ROOT, rid, sid]
					if ResourceLoader.exists(p_c) or FileAccess.file_exists(p_c): return p_c
				elif clean_id.find("stock") >= 0:
...
static func get_sorted_slot_entries_512(race: String, slot_selection: Dictionary = {}) -> Array[Dictionary]:
	...
	for slot_def in sorted_slots:
		...
		# 耳朵／頭部要跟塗裝同色，否則身體換色但頭部脫節
		if sid == SLOT_HEAD_UNIT:
			var paint := str(slot_selection.get(SLOT_CHASSIS, slot_selection.get("paint_id", "")))
			var head_variant := ""
			if rid == "rabbit":
				if paint.find("midnight") >= 0 or paint.find("navy") >= 0:
					head_variant = "%s/%s/%s/ear_rabbit_straight_midnight_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("brass") >= 0:
					head_variant = "%s/%s/%s/ear_rabbit_straight_brass_512.png" % [PAPERDOLL_ROOT, rid, sid]
			elif rid == "bear":
				if paint.find("amber") >= 0:
					head_variant = "%s/%s/%s/head_iron_bear_amber_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("quarry") >= 0 or paint.find("iron") >= 0:
					head_variant = "%s/%s/%s/head_iron_bear_quarry_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("ivory") >= 0 or paint.find("stock") >= 0:
					head_variant = "%s/%s/%s/head_iron_bear_stock_512.png" % [PAPERDOLL_ROOT, rid, sid]
			elif rid == "penguin":
				if paint.find("navy") >= 0 or paint.find("penguin_navy") >= 0:
					head_variant = "%s/%s/%s/head_steam_penguin_stock_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("polar") >= 0 or paint.find("frost") >= 0:
				...
				if paint.find("midnight") >= 0 or paint.find("navy") >= 0:
					...
					head_variant = "%s/%s/%s/ear_lion_gilded_mane_brass_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
				...
				elif paint.find("orange") >= 0:
					head_variant = "%s/%s/%s/ear_fox_radar_orange_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
				...
				if paint.find("crimson") >= 0 or paint.find("molten") >= 0:
					...
					head_variant = "%s/%s/%s/ear_boar_rivet_cowl_ivory_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
				...
				if paint.find("bronze") >= 0 or paint.find("bamboo") >= 0:
					...
					head_variant = "%s/%s/%s/ear_macaque_coaxial_ivory_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
				...
				elif paint.find("ember") >= 0 or paint.find("orange") >= 0:
					head_variant = "%s/%s/%s/head_ember_tiger_ember_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
				...
				elif paint.find("porcelain") >= 0:
					head_variant = "%s/%s/%s/head_cloud_crane_porcelain_512.png" % [PAPERDOLL_ROOT, rid, sid]
				elif paint.find("stock") >= 0:
...
## 依據種族、幀數 (0..3) 與換裝選擇，執行期分部位即時合成走路姿態（512 高清，失敗退回預設 512，絕不退回 128/x3）
static func build_walk_composite_image(race: String, frame: int, slot_selection: Dictionary = {}) -> Image:
	...
	if img_512 != null and not img_512.is_empty():
		return img_512
	# 512 合成失敗時退回該族預設 512 走路幀，絕不退回 128/x3
	if not slot_selection.is_empty():
		var def_img := build_walk_composite_image_512(race, frame, {})
		if def_img != null and not def_img.is_empty():
			return def_img
	return null
...
static func build_walk_composite_texture(race: String, frame: int, slot_selection: Dictionary = {}) -> Texture2D:
	...
	if tex_512 != null:
		return tex_512
	if not slot_selection.is_empty():
		var def_512 := build_walk_composite_texture_512(race, frame, {})
		if def_512 != null:
			return def_512
	return null
