...
static func player_equipped_walk(frame: int, race_override: String = "", slots_override: Dictionary = {}) -> Texture2D:
	...
	var pr: GDScript = load("res://scripts/art/paperdoll_renderer.gd")
	if pr and pr.has_method("build_walk_composite_texture_512"):
...
static func speaker_portrait(speaker: String) -> Texture2D:
	...
	match key:
		...
		"魔王", "停擺核", "停摆核", "Stasis Core", "Núcleo de Inactividad", "停止核", "정지핵", "demon":
			id = "demon"
		"渣滓之狼", "失控的鏽蝕玩具", "失控的锈蚀玩具", "Rust-bound Toy", "Juguete Oxidado Descontrolado", "暴走した錆びトイ", "폭주한 녹슨 장난감", "狼", "wolf":
			id = "wolf"
		"潮吼", "潮聲", "tide_roar":
			id = "tide_roar"
