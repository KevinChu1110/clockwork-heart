extends SceneTree
## 《發條之心》烈焰虎族第二套外裝/塗裝變體 (PaperdollCharacter - Tiger Variants) 無頭驗證測試
## 執行方式：godot --path game --headless -s res://scripts/art/test_paperdoll_tiger_variants.gd
...
func _initialize() -> void:
	var ok := true
	print("=== 開始烈焰虎族第二套塗裝/外裝變體無頭驗證 (Tiger Variants Test) ===")
	...
	# 1. 測試新外裝 costume_ash_ninja_garb
	print("\n--- 1. 驗證新外裝 costume_ash_ninja_garb ---")
	character.render_character("tiger", {"costume": "costume_ash_ninja_garb"})
	var c_sprite: Sprite2D = character.get_slot_sprite("costume")
	if c_sprite == null or c_sprite.texture == null:
		push_error("costume_ash_ninja_garb 貼圖載入失敗！")
		ok = false
	else:
		var sz := c_sprite.texture.get_size()
		print("  ✓ costume_ash_ninja_garb 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(c_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("costume_ash_ninja_garb 尺寸不為 128x128")
			ok = false

	# 2. 測試新塗裝 paint_volcano_black
	print("\n--- 2. 驗證新塗裝 paint_volcano_black ---")
	character.render_character("tiger", {"chassis": "paint_volcano_black"})
	var ch_sprite: Sprite2D = character.get_slot_sprite("chassis")
	if ch_sprite == null or ch_sprite.texture == null:
		push_error("paint_volcano_black 貼圖載入失敗！")
		ok = false
	else:
		var sz := ch_sprite.texture.get_size()
		print("  ✓ paint_volcano_black 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(ch_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("paint_volcano_black 尺寸不為 128x128")
			ok = false

	# 3. 測試雙新變體同時混搭合成 (volcano_black + ash_ninja_garb)
	print("\n--- 3. 驗證雙新變體同時混搭合成 (volcano_black + ash_ninja_garb) ---")
	character.render_character("tiger", {
	...
	character.render_character("tiger", {"costume": "none"})
	...
	# 5. 測試原廠塗裝 + 新外裝混搭 (ember_orange + ash_ninja_garb)
	print("\n--- 5. 驗證原廠塗裝 + 新外裝混搭 (ember_orange + ash_ninja_garb) ---")
	character.render_character("tiger", {
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("TIGER_VARIANTS_TEST_OK")
		quit(0)
	else:
		print("TIGER_VARIANTS_TEST_FAIL")
		quit(1)
