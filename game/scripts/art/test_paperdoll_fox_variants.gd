extends SceneTree
## 《發條之心》狐族第二套外裝/塗裝變體 (PaperdollCharacter - Fox Variants) 無頭驗證測試
## 執行方式：godot --path game --headless -s res://scripts/art/test_paperdoll_fox_variants.gd
...
func _initialize() -> void:
	var ok := true
	print("=== 開始狐族第二套塗裝/外裝變體無頭驗證 (Fox Variants Test) ===")
	...
	# 1. 測試新外裝 costume_astral_observer
	print("\n--- 1. 驗證新外裝 costume_astral_observer ---")
	character.render_character("fox", {"costume": "costume_astral_observer"})
	var c_sprite: Sprite2D = character.get_slot_sprite("costume")
	if c_sprite == null or c_sprite.texture == null:
		push_error("costume_astral_observer 貼圖載入失敗！")
		ok = false
	else:
		var sz := c_sprite.texture.get_size()
		print("  ✓ costume_astral_observer 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(c_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("costume_astral_observer 尺寸不為 128x128")
			ok = false

	# 2. 測試新塗裝 paint_emerald_glaze
	print("\n--- 2. 驗證新塗裝 paint_emerald_glaze ---")
	character.render_character("fox", {"chassis": "paint_emerald_glaze"})
	var ch_sprite: Sprite2D = character.get_slot_sprite("chassis")
	if ch_sprite == null or ch_sprite.texture == null:
		push_error("paint_emerald_glaze 貼圖載入失敗！")
		ok = false
	else:
		var sz := ch_sprite.texture.get_size()
		print("  ✓ paint_emerald_glaze 成功載入，貼圖尺寸: %s, 可見度: %s" % [str(sz), str(ch_sprite.visible)])
		if int(sz.x) != 128 or int(sz.y) != 128:
			push_error("paint_emerald_glaze 尺寸不為 128x128")
			ok = false

	# 3. 測試雙新變體同時混搭合成 (emerald_glaze + astral_observer)
	print("\n--- 3. 驗證雙新變體同時混搭合成 (emerald_glaze + astral_observer) ---")
	...
	character.render_character("fox", {"costume": "none"})
...
func _finish(ok: bool) -> void:
	print("\n=======================================================")
	if ok:
		print("FOX_VARIANTS_TEST_OK")
		quit(0)
	else:
		print("FOX_VARIANTS_TEST_FAIL")
		quit(1)
