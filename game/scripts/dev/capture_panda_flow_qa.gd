extends SceneTree
## 瓷韻熊貓（第十三族）全流程實機截圖產生器 (創角、戰鬥、大廳與特寫裁剪)
## 執行方式：
## xvfb-run -a godot --path game --rendering-driver opengl3 -s res://scripts/dev/capture_panda_flow_qa.gd
	...
	match _step:
		1:
			...
			if _wait_frames == 1:
				...
				_save_screenshot("proof_creation_panda.png")
				print("  ✓ 步驟 1 完成：創角畫面選中瓷韻熊貓 -> proof_creation_panda.png")
				if _current_node:
		...
		2:
			# 步驟 2: 建立戰鬥畫面 (BattleView) 並載入瓷韻熊貓
			if _wait_frames == 1:
				var gs: Node = root.get_node_or_null("GameState")
				if gs:
					gs.call("reset_new_game", "panda")
				...
				_save_screenshot("proof_battle_panda.png")
				print("  ✓ 步驟 2 完成：戰鬥畫面瓷韻熊貓出戰 -> proof_battle_panda.png")
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 3
		...
		3:
			# 步驟 3: 建立手遊大廳 (MobileLobby) 並展示瓷韻熊貓
			if _wait_frames == 1:
				var gs: Node = root.get_node_or_null("GameState")
				if gs:
					gs.call("reset_new_game", "panda")
				...
				_save_screenshot("proof_lobby_panda.png")
				print("  ✓ 步驟 3 完成：手遊大廳瓷韻熊貓展示 -> proof_lobby_panda.png")
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step = 4
...
func _save_screenshot(filename: String) -> void:
	...
	if err == OK:
		...
		img.save_png(proofs_dir.path_join(filename))
	else:
...
func _generate_crops() -> void:
	...
	var battle_path := _out_dir.path_join("proof_battle_panda.png")
	if FileAccess.file_exists(battle_path):
		...
		if full_img != null and not full_img.is_empty():
			...
			var crop_path := _out_dir.path_join("proof_crop_battle_panda.png")
			crop_img.save_png(crop_path)
			var proofs_dir := ProjectSettings.globalize_path("res://").path_join("../proofs")
			crop_img.save_png(proofs_dir.path_join("proof_crop_battle_panda.png"))
			print("  ✓ 已儲存戰鬥角色特寫: %s" % crop_path)

	# 大廳中角色本體特寫 (X:515, Y:310, W:250, H:270)
	var lobby_path := _out_dir.path_join("proof_lobby_panda.png")
	if FileAccess.file_exists(lobby_path):
		...
		if full_lobby != null and not full_lobby.is_empty():
			...
			var lobby_crop_path := _out_dir.path_join("proof_crop_lobby_panda.png")
			lobby_crop_img.save_png(lobby_crop_path)
			var proofs_dir := ProjectSettings.globalize_path("res://").path_join("../proofs")
			lobby_crop_img.save_png(proofs_dir.path_join("proof_crop_lobby_panda.png"))
			print("  ✓ 已儲存大廳角色特寫: %s" % lobby_crop_path)

	# 紙娃娃素體合成
	var comp_tex := SpriteDB.player_race_composite("panda")
	if comp_tex != null:
		var comp_img := comp_tex.get_image()
		if comp_img != null and not comp_img.is_empty():
			var comp_path := _out_dir.path_join("proof_paperdoll_composite_panda.png")
			comp_img.save_png(comp_path)
			var proofs_dir := ProjectSettings.globalize_path("res://").path_join("../proofs")
			comp_img.save_png(proofs_dir.path_join("proof_paperdoll_composite_panda.png"))
