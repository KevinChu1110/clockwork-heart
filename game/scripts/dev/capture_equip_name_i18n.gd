extends SceneTree
## 裝備與武器名稱六語系實機截圖腳本 (equip-name-i18n)
## 嚴格遵循：review.md 0-QA15, 0-QA23, 0-QA24, 0-QA25
## 驗收重點：
## 1. 角色分頁全景在 en、ja、zh_TW 下實機渲染。
## 2. 彈窗與大廳 chrome 底層（頂欄狀態、出征、底部 Dock 等）同步切換至同一語系 (0-QA25)。
## 3. 實機截圖輸出目錄嚴格限定為 proofs/equip-name-i18n/ (0-QA23)。
## 4. 局部 Crops 供顯微比對武器槽與屬性小卡。

const OUT_DIR := "proofs/equip-name-i18n"
const CROPS_DIR := "proofs/equip-name-i18n/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行裝備與武器名稱六語系實機截圖腳本 (equip-name-i18n) ──")
	_step = 1
	_wait = 0
