extends SceneTree
## 鋼岳象玩家可見名稱六語系落地實機截圖腳本 (elephant-i18n)
## 驗收重點：
## 1. 創角擴充分頁選鋼岳象，展示【鋼岳象】、【戰士】、【巨輪工坊厚鋼工裝】、【原廠巨輪工坊黃銅原金】在 zh_TW, en, ja 下即時切換。
## 2. 衣櫥選用鋼岳象外裝／塗裝標籤，在大廳背景與底部 Dock 連動下（0-QA25），在 zh_TW, en, ja 下即時切換。

const OUT_DIR := "/opt/side/bravesoul-game/proofs/elephant-i18n"
const CROPS_DIR := "/opt/side/bravesoul-game/proofs/elephant-i18n/crops"
...
func _initialize() -> void:
	...
	print("── 開始執行鋼岳象六語系落地實機截圖腳本 (elephant-i18n) ──")
	_step = 1
	_wait = 0
...
func _process(_delta: float) -> bool:
	...
	match _step:
		1, 2, 3:
			# 步驟 1~3: 創角介面 (PaperdollSelectDemo) 擴充分頁選鋼岳象 (zh_TW, en, ja)
			var loc_idx := _step - 1
			...
			if _wait == 1:
				...
				var path := "%s/proof_creation_elephant_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 創角介面選鋼岳象 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
		...
		4, 5, 6:
			# 步驟 4~6: 大廳開啟衣櫥 (WardrobeDialog)，篩選「象」展示工裝與塗裝標籤，背景大廳與 Dock 同步切換語系 (0-QA25)
			var loc_idx := _step - 4
			var code: String = TEST_LOCALES[loc_idx]
			if _wait == 1:
				if _loc_node:
				...
				if _current_node and _current_node.has_method("open_wardrobe"):
					...
					if wardrobe and wardrobe.has_method("set_race_filter"):
						wardrobe.call("set_race_filter", "elephant")
			elif _wait >= 40:
				var path := "%s/proof_wardrobe_elephant_%s.png" % [OUT_DIR, code]
				_save_screenshot(path)
				print("  ✓ [%d/6] 大廳連動衣櫥象族標籤 [%s] 實機截圖完成: %s" % [_step, code, path])
				if _current_node:
					_current_node.queue_free()
					_current_node = null
				_step += 1
...
func _generate_crops() -> void:
	# 裁切創角右側資訊面板文字區 (X:760~1260, Y:100~580) 驗證四詞
	for code in TEST_LOCALES:
		var c_path := "%s/proof_creation_elephant_%s.png" % [OUT_DIR, code]
		...
		# 裁切衣櫥卡片標籤與左側徽章區 (X:220~920, Y:150~600)
		var w_path := "%s/proof_wardrobe_elephant_%s.png" % [OUT_DIR, code]
