...
var _out_dirs: Array[String] = [
	"/opt/side/bravesoul-game/proofs/enemy_audit",
...
func _initialize() -> void:
	print("CAPTURE_FOG_HIDE: Initializing...")
	...
	print("CAPTURE_FOG_HIDE: change_scene_to_file err=", err)
	_phase = 0
	_wait = 0
	...
	match _phase:
		0:
			if _wait >= 40:
				...
				print("CAPTURE_FOG_HIDE: Main scene is ", _main.name if _main else "null")
				_setup_player_env()
				_phase = 1
				_wait = 0
		1:
			if _wait >= 15:
				print("CAPTURE_FOG_HIDE: Playing dialogue for 霧隱...")
				var lines := [
					{"speaker": "霧隱", "text": "霧裡真假同色。發條最鬆的那掛？……眼睛，借我用用。"}
				]
		...
		2:
			...
			if _wait >= 30:
				_save_viewport("proof_dialogue_fog_hide.png", "Dialogue - 霧隱")
				_phase = 3
				_wait = 0
		3:
			if _wait >= 10:
				print("CAPTURE_FOG_HIDE: Complete, quitting.")
				quit(0)
				return true
	return false
