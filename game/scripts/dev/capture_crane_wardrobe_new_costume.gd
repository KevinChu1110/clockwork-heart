extends SceneTree
## 執行方式：xvfb-run -a godot --path game --rendering-driver opengl3 -s res://scripts/dev/capture_crane_wardrobe_new_costume.gd
...
func _initialize() -> void:
	...
	_out_dir = base.path_join("../proofs/wardrobe_crane")
	...
	if gs:
		...
		gs.paperdoll_slots = {
			...
			"costume_id": "costume_sky_hunter_mail",
			"paint_id": "paint_zephyr_azure",
		...
		1:
			if _wait_frames == 10:
				if _lobby != null:
					...
					if _wardrobe != null:
						# Select Sky Hunter Mail & Zephyr Azure
						...
						print("  [Wardrobe] 已開啟衣櫥並選中【晴空巡獵機關羽甲】與【晴空凌雲湛藍】")
			if _wait_frames >= 40:
				var fn := "proof_wardrobe_crane_sky_hunter.png"
				_save_screenshot(fn)
				print("  ✓ [1/2] 雲嵐鶴穿戴【晴空巡獵機關羽甲】衣櫥彈窗截圖完成 -> %s" % fn)
				...
				if _wardrobe != null and is_instance_valid(_wardrobe):
					...
					if _wardrobe.has_method("_update_preview"):
						_wardrobe.call("_update_preview")
					print("  [Wardrobe] 已切換回【凌雲羽衣輕鋼道袍】與【原廠冷淬青瓷白】")
				_step = 2
				_wait_frames = 0
		2:
			if _wait_frames >= 30:
				var fn2 := "proof_wardrobe_crane_zephyr_robe.png"
				_save_screenshot(fn2)
				print("  ✓ [2/2] 雲嵐鶴穿戴【凌雲羽衣輕鋼道袍】衣櫥彈窗截圖完成 -> %s" % fn2)
				if _wardrobe != null and is_instance_valid(_wardrobe):
