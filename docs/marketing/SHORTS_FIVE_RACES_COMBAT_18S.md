...
|---|---|---|---|---|---|---|---|---|
| **REC-01** | 白金兔 晨光長劍突刺 | `docs/marketing/shots/rec01_rabbit_combat_9x16.mp4` | `docs/marketing/shots/rec01_rabbit_combat_raw_16x9.mp4` | 260,950 B / 275,985 B | ✅ aac, 44.1kHz, 立體聲 (`slash`+`hit`) | ✅ `28.04 dB` (實體位移) | ✅ `f1d9ce61` / `3db8513e` / `ba4fabd9`<br>(diff: 42,833 px) | ✅ 右手握持晨光長劍劍柄，金色十字護手緊貼手腕 |
| **REC-02** | 烈鬃獅 皇家長槍突貫 | `docs/marketing/shots/rec02_lion_combat_9x16.mp4` | `docs/marketing/shots/rec02_lion_combat_raw_16x9.mp4` | 170,522 B / 168,145 B | ✅ aac, 44.1kHz, 立體聲 (`clash`+`hit`+`slash`) | ✅ `24.72 dB` (長槍突貫) | ✅ `73cc8ad9` / `7227cf14` / `bfceaa2e`<br>(diff: 66,111 px) | ✅ 右手緊扣皇家長槍護手盤正下方握柄，槍身穿出 |
| **REC-03** | 靈尾狐 星盤秘術法杖爆破 | `docs/marketing/shots/rec03_fox_combat_9x16.mp4` | `docs/marketing/shots/rec03_fox_combat_raw_16x9.mp4` | 213,275 B / 221,180 B | ✅ aac, 44.1kHz, 立體聲 (`wind`+`hit`+`slash`+`fire`) | ✅ `26.44 dB` (能量衝擊) | ✅ `734e1085` / `585b0afd` / `04f305ef`<br>(diff: 51,587 px) | ✅ 左手五指環扣星盤法杖杖身，頂部青綠水晶發光 |
| **REC-04** | 鋼牙豕 鍛爐鐵砧重型戰鎚砸地 | `docs/marketing/shots/rec04_boar_combat_9x16.mp4` | `docs/marketing/shots/rec04_boar_combat_raw_16x9.mp4` | 222,647 B / 215,633 B | ✅ aac, 44.1kHz, 立體聲 (`slash`+`rock`+`hit`+`break`) | ✅ `26.19 dB` (戰鎚重砸) | ✅ `477d6854` / `642406f8` / `ddbff530`<br>(diff: 48,744 px) | ✅ 雙手握持鍛爐戰鎚長柄，指節覆蓋握柄上方 |
| **REC-05** | 靈爪猴 機關發條靈爪連擊 | `docs/marketing/shots/rec05_macaque_combat_9x16.mp4` | `docs/marketing/shots/rec05_macaque_combat_raw_16x9.mp4` | 271,778 B / 272,815 B | ✅ aac, 44.1kHz, 立體聲 (`slash`+`hit`+`wind`) | ✅ `29.92 dB` (狂暴快爪) | ✅ `d2a81bdc` / `cecc6dad` / `093370c0`<br>(diff: 68,929 px) | ✅ 雙腕裝備機關發條靈爪護手，直拳出擊與收拳蓄勢衝擊 |
...
| 實機序列 | 測試種族與模式 | 裝備 ID (bases) | 我方首個 attack_swing (sim.time) | 我方首個 hit (sim.time / 傷害) | 敵方先手 (swing / hit) | skill_hit 次數 | fury_awakening 次數 | 戰鬥狀態說明 |
|---|---|---|---|---|---|---|---|---|
| **REC-01** | 兔族 vs 荒路殘兵 (`road_bandit`) | `dawn_blade` | `sim.time = 4.00s` | `sim.time = 4.27s` / 32～38 點 | 4.00s / 4.27s（同拍互毆） | 0 次（平砍無技能） | 0 次 | 首次命中 4.27s，之後戰鬥可能延續（實測結束時間 9.03～13.80s 浮動），錄影窗不依賴擊殺 |
| **REC-02** | 獅族 vs 黑鏽浪人 (`black_ronin`) | `knight_pike` | `sim.time = 4.00s` | `sim.time = 4.28s` / 12～22 點 | 3.65s / 3.92s（敵方先手） | 0 次（平砍無技能） | 0 次 | 持續對戰中（實測 21.58s 結束）。我方首擊可能 miss，三輪中有一輪首次命中落到 9.00s |
| **REC-03** | 狐族 vs 霧影 (`fog_shade`) | `star_rod` | `sim.time = 4.00s` | `sim.time = 4.33s` / 約 15 點 | 3.58s / 3.85s（敵方先手） | 0 次（平砍無技能） | 0 次 | 持續對戰中（實測 25.60s 結束） |
| **REC-04** | 豬族 vs 潮襲海盜 (`coast_raider`) | `anvil_hammer` | `sim.time = 4.00s` | `sim.time = 4.42s` / 約 13 點 | 4.00s / 4.27s（同拍出手，敵方先命中） | 0 次（平砍無技能） | 0 次 | 持續對戰中（實測 23.33～28.10s 結束） |
| **REC-05** | 猴族 vs 竹影拳靈 (`bamboo_spirit`) | `hunt_claw` | `sim.time = 2.87s`（狂暴攻速） | `sim.time = 3.03s` / 28～56 點（可暴擊） | 3.85s / 4.12s（我方先手） | 0 次（狂暴連爪） | 1 次（0.00s 手動觸發） | 實測 6.42s 結束，錄影窗須在 6.4s 前收完 |

> ⚠️ **判讀規則（第 19e-5 條）**：`attack_swing` 事件用 `data.id`、`hit` 事件用 `data.attacker` 區分我方與敵方。獅、狐兩場是**敵方先出手**（3.65s／3.58s），那不是我方動作；上一輪把敵方出手誤填成我方出手，本輪已依「我方事件」重測回填。傷害值受 rng 與暴擊影響會浮動，表中給區間，⛔ 不要寫成固定值。

- **實測結論（落實第 19e-5 條）**：
  1. 我方普通攻擊的第一個 `attack_swing`（`data.id == "player"`）實測完全可重現：兔族 4.00s→4.27s、獅族 4.00s→4.28s、狐族 4.00s→4.33s、豬族 4.00s→4.42s、猴族（狂暴）2.87s→3.03s。⚠️ 敵方的先手出手時間才是各族有差異的那組（獅 3.65s／狐 3.58s／猴 3.85s），錄影窗要把敵方先手一起框進來畫面才有來回。
  2. 觀察窗全面排除測試內部幀數計數，統一改以 `sim.time` 作為判定與控制依據，錄影腳本應以 `while sim.time < start_t: step()` 與 `while sim.time < end_t: step()` 精確錄製。
  3. 各族擊殺與傷害值受暴擊 rng 影響（兔族實測 9.03～13.80s 結束、豬族 23.33～28.10s），錄影窗一律鎖在出手與首次命中反饋區間，不依賴擊殺結束。猴族 6.42s 結束，錄影窗須在此之前收完。
