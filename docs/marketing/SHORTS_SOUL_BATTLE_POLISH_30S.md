...
1. **視覺換血（告別粗顆粒）**：
   - 全面去像素量化，手繪插畫質感渲染。
   - 角色深色厚描邊（`character_outline.gdshader`）＋ 獨立層落地柔化橢圓軟影（`foot_shadow.gdshader`），角色輪廓立體、腳踏實地不飄浮。
2. **手感進化（單手輕鬆打）**：
   - 右手拇指人體工學熱區（`ThumbPad`：攻擊、換武、鎖定、技能、暫停），熱區 ≥50px，單手拇指打完一場。
   - 0.15s 打擊停頓時間模型（Hitstop），受擊震顫與金光暴擊跳字，刀刀有肉、金屬反饋扎實。
3. **戰魂收斂（告別同質星盤）**：
   - 徹底拆除 14 主星同質冗餘，收斂為**四大機械共鳴核心**（【銳齒】增攻、【固甲】減傷、【旋簧】擴血、【全衡】三圍）。
   - 聚魂殿四階金屬發條封靈罐（綠→藍→紫→橙），首屏透明保底進度條，流派一目了然。

---

## 三、接鏡軸線與運鏡節奏設計

本片拒絕靜圖硬切與 PPT 投影片感，採用「微距懸念 → 實機指尖操作 → 打擊爆發 → 系統回饋 → 終極過載 → 品牌點題」的連續鏡頭流：
...
Shot 2 (3.0-7.5s)  【實機·操控】神殿荒路戰鬥，單手右手拇指熱區連點：長劍突刺＋換武
        │ (揮劍橫斬動態接刀，鏡頭跟隨劍尖衝擊)
Shot 3 (7.5-13.0s) 【實機·視覺】雷歐戰平視特寫：厚描邊＋落地柔化軟影＋0.15s 打擊停頓火花
        │ (劍刃擊破金光，金色光弧向下注入封靈罐)
Shot 4 (13.0-18.0s)【實機·養成】聚魂殿封靈罐開光金芒，鏡頭推入展示首屏透明保底進度條
        │ (罐口金光展開為圖鑑卡片網格)
Shot 5 (18.0-23.0s)【實機·圖鑑】戰魂圖鑑橫向滑移，展示四大共鳴核心（銳齒/固甲/旋簧/全衡）
        │ (核心光芒注入武器魂槽，切回戰鬥)
Shot 6 (23.0-27.0s)【實機·高潮】怒氣滿 100%，一指點擊 Overdrive 齒輪過載超轉速狂暴斬擊！
        │ (最後一記重劈震屏白閃，瞬間切純黑)
Shot 7 (27.0-30.0s)【合成·點題】純黑底板微推，手繪字標《發條之心》金屬微光掃過，發條定格
```
...
| **Shot 3<br>【打擊換血】** | `7.5-13.0s`<br>(5.5s) | 小白正面交鋒守衛巨獸。畫面細節特寫：告別 16x16 粗像素方塊，呈現通透平滑插畫感。角色身上帶有深暖褐粗描邊（`outline`），腳底踩著清晰貼地的獨立柔化橢圓軟影（`foot_shadow`）。長劍斬中，觸發精確 0.15s 打擊停頓（Hitstop），爆出金色齒輪火花。 | 隨武器劈砍軌跡向左下微幅下搖（Pan Down），命中瞬間觸發 0.15s 畫面急停微震，緊接著火花爆散。 | 沉重鈍擊與金屬切割交錯聲「鏘——轟！」，0.15s 命中停頓瞬間聲場短暫抽真空，隨後火花炸裂。 | 【全新打擊手感】<br>去像素插畫感 × 落地柔影 × 0.15s 停頓 | **【實機素材·REC-02】**<br>錄製自 Godot 實機 `res://scenes/battle/battle.tscn`（`setup("leo")` 或 `setup("road_bandit")`）。<br>展示已合併 main 的 `character_outline.gdshader` 與 `foot_shadow.gdshader`。 |
| **Shot 4<br>【封靈開光】** | `13.0-18.0s`<br>(5.0s) | 戰鬥火花化為聚魂殿金光。展示四大殿堂之「聚魂殿」。中央黃銅發條封靈罐（無任何系統 Emoji）震動進階，金光滿溢。鏡頭帶出畫面上方直觀透明的「首屏保底進度條」（60/100 虔誠度，4 碎片明確標示），點擊抽魂，橙罐爆裂開光！ | 鏡頭自全景向中央封靈罐平滑推入（Push In），光芒爆發時輕微拉遠展現完整 UI 介面。 | 發條罐劇烈上弦發條聲「嘸——咔啦啦」，金屬鎖扣彈開聲，開光爆發的清脆天籟鐘鳴「鐺——」。 | 【透明首屏保底】<br>發條封靈罐 · 告別隱形坑 | **【實機素材·REC-03】**<br>錄製自 Godot 實機 `res://scenes/main.tscn` 之 `_go_soul_panel()`。<br>展示已合併 main 的首屏保底進度條與封靈罐系統。 |
| **Shot 5<br>【四大核心】** | `18.0-23.0s`<br>(5.0s) | UI 介面無縫滑動切入「戰魂圖鑑」（Soul Codex）。畫面列出收斂後的「四大機械共鳴核心」：【銳齒之魂】（極致暴力攻擊）、【固甲之魂】（厚實護甲減免）、【旋簧之魂】（強韌生命擴充）、【全衡之魂】（攻防均衡全面）。橙色神品戰魂卡牌帶有黃銅齒輪邊框，流派偏科清爽明瞭。 | UI 水平微幅橫移（Slow Pan Right），當滑過各核心卡片時卡片微幅浮起高亮。 | 齒輪定位嵌合聲「卡嗒、卡嗒、卡嗒」，四聲節奏分明的入位音。 | 【四大共鳴核心】<br>拆除十四同質星 · 攻防血衡一眼懂 | **【實機素材·REC-04】**<br>錄製自 Godot 實機 `res://scenes/main.tscn` 之 `_go_astrolabe_panel()`。<br>展示已合併 main 的四核心圖鑑與品質倍率系統。 |
| **Shot 6<br>【過載狂斬】** | `23.0-27.0s`<br>(4.0s) | 核心歸位，畫面急切回戰鬥現場！怒氣累積滿 100%，右下角 `ThumbSkill` / `PlayerRage` 爆散彩糖星芒。一指點下，小白安全閥掀開，蒸氣噴薄，啟動「齒輪過載（Overdrive）」，攻速暴增 25%！長劍化為漫天金屬金光，BOSS 部位瞬間承受連環重擊，打出金字「BREAK！」零件崩飛！ | 鏡頭拉近平視動態跟隨，刀光閃爍伴隨劇烈節奏微震，部位擊破時畫面短暫放射狀模糊（Radial Blur）。 | 高壓蒸氣劇烈排氣「嘶——！」，發條超轉速蜂鳴，密集成串的金屬暴擊「乒乒乓乓！」，部位崩落的金屬碎裂聲。 | 【超轉速過載】<br>部位拆卸 · 一指逆轉戰局！ | **【實機素材·REC-05】**<br>錄製自 Godot 實機 `res://scenes/battle/battle.tscn`。<br>展示已合併 main 的怒氣暴怒覺醒（`trigger_fury_awakening`）與部位破壞跳字。 |
| **Shot 7<br>【點題收束】** | `27.0-30.0s`<br>(3.0s) | 漫天火花切純黑。純黑點題底板（`branding/title_plate.png`）中央顯現手繪繪本風主字標《發條之心》（`branding/logo_cn.png`）。下方標語：「給心上弦，重新出發。」右下角顯現小白黃銅發條鑰匙剪影。底部標註：「開發中畫面 · 官網搶先看」。 | 純黑底板微幅向前緩推 102%，字標金屬邊緣一道柔和暖金高光緩慢掃過。 | 一聲清脆悠長、餘音裊裊的單聲「喀嗒」。全片安靜收尾。 | 發條之心 · Clockwork Heart<br>官網搶先看（連結在簡介） | **【固定資產·合成】**<br>使用官方固定品牌資產 `branding/logo_cn.png` 與 `title_plate.png`。<br>⚠️ 依第 15/16 條：不放下載鈕、不承諾上市日期、不提價格。 |

- **秒數累計核對**：`3.0 + 4.5 + 5.5 + 5.0 + 5.0 + 4.0 + 3.0 = 30.0 秒`（精準嚴絲合縫）。
...
> 所有標示「實機」之素材，**必須能在既有 Godot 引擎中重現與錄製，且功能已合併進 main**。嚴禁以 AI 主視覺充當實機，嚴禁列出「之後再做」的幽靈功能。

以下為本片所需之 5 支實機錄影片段規格與操作序列清單：
...
- **對應分鏡**：Shot 3 (7.5s～13.0s)
- **所屬功能**：`game-battle-polish`（戰鬥畫面優化，main 合併 commit `3a801b6`）
- **Godot 場景路徑**：`res://scenes/battle/battle.tscn`
- **對應著色器與腳本**：
  - `game/shaders/character_outline.gdshader`（角色深色描邊，寬度 2.5px，色值 `#331F12`）
...
- **對應分鏡**：Shot 4 (13.0s～18.0s)
- **所屬功能**：`soul-simplify`（戰魂收斂與首屏保底，main 合併 commit `0736a53` / `d8a9aa5`）
- **Godot 場景路徑**：`res://scenes/main.tscn`
- **對應腳本**：`game/scripts/systems/soul_system.gd`、`game/scripts/dev/capture_soul_simplify.gd`
...
### 【實機素材 4：REC-04 戰魂圖鑑四大共鳴核心（銳齒/固甲/旋簧/全衡）】
- **對應分鏡**：Shot 5 (18.0s～23.0s)
- **所屬功能**：`soul-simplify`（14 主星拆除與戰魂圖鑑，main 合併設計文件 `docs/SOUL_SIMPLIFY_DESIGN.md`）
- **Godot 場景路徑**：`res://scenes/main.tscn`
- **對應腳本**：`game/scripts/systems/soul_system.gd`（`survey_astrolabe()` 轉型戰魂圖鑑）
- **參考現存截圖**：`screenshots/soul_codex.png`、`screenshots/soul_codex_details.png`
- **錄製操作序列**：
  1. 在聚魂殿點擊「戰魂圖鑑」或呼叫 `_go_astrolabe_panel()`。
  2. 錄製平滑向下捲動展示四大共鳴核心：
     - **【銳齒之魂】（Razor Tooth Core）**：專精攻擊力加成，部位破壞專用。
     - **【固甲之魂】（Plated Iron Core）**：專精護甲減傷，正面硬抗巨獸。
     - **【旋簧之魂】（Coiled Spring Core）**：專精生命上限，持久戰續航。
     - **【全衡之魂】（Grand Balance Core）**：三圍均衡提升，全能萬用。
  3. 展示六階品質乘數（大凶 0.4 ～ 神 4.5）與金色神魂卡牌圖鑑點亮反饋。
- **預期畫面**：徹底告別傳統紫微斗數 14 星同質文字混亂，分類極致清爽，流派特徵一眼看懂。

### 【實機素材 5：REC-05 怒氣滿額超轉速過載（Overdrive）與部位 BREAK】
- **對應分鏡**：Shot 6 (23.0s～27.0s)
...
- **錄製操作序列**：
  ...
  5. 小白以 +25% 攻擊速度進行高速 4 連斬，最後一記重擊打碎雷歐獅鬃裝甲，彈出金色「BREAK！」字樣與齒輪零件四散爆散。
...
| **第 12 條** | **鏡次接得上** | 嚴格維持接鏡動態軸線：微距光芒穿透 → 實機指尖操作 → 揮斬震顫 → 開光爆發展示保底 → 核心滑移圖鑑 → 終結過載 → 點題收束。 | ✅ 合格 |
| **第 13 條** | **標「不移動」的鏡次真的不走** | Shot 1 標明小白原位靜止（僅鑰匙旋轉與核心充能），Shot 7 標明純黑定格微推，無任意漫步。 | ✅ 合格 |
...
| **第 16 條** | **不暗示已完成，不拿 AI 圖充當實機** | 概念鏡次（Shot 1）與合成鏡次（Shot 7）明確標示為概念與品牌字標；實機鏡次（Shot 2～6）100% 來自真實 Godot 錄影。 | ✅ 合格 |
| **第 17 條** | **切入點獨立，風格不撞車** | 本片切入點為純粹的「戰鬥手感與系統收斂爽感」，與 C0 故事短片《喀嗒》、品牌概念片《覺醒》調性截然不同。 | ✅ 合格 |
| **第 19 條** | **素材列出必須是已存在之合併功能** | 所列 5 項實機素材（REC-01 至 REC-05）皆對應 main 上已合併之 commit（`0975523`、`3a801b6`、`0736a53` 等）。 | ✅ 合格 |
| **第 19b 條** | **標實機的必須真的是 Godot 錄影** | 實機清單列出精確 Godot 場景路徑（`scenes/battle/battle.tscn`、`scenes/main.tscn`）、操作序列及測試代碼對應。 | ✅ 合格 |
| **第 22 條** | **不違背 CANON 世界憲章** | 嚴格遵守「覺醒的金屬發條玩具」世界觀：零毛皮、背後鑰匙、胡桃鉗色盤、齒輪部位破壞無血肉、舊 IP 詞彙全面掃蕩。 | ✅ 合格 |
| **第 23 條** | **名詞六語系皆可翻譯** | 核心專有名詞：銳齒（Razor Tooth）、固甲（Plated Iron）、旋簧（Coiled Spring）、全衡（Grand Balance）、封靈罐（Spirit-Seal Jar），六語系均已完成對齊。 | ✅ 合格 |

---

## 八、下一步實作指引（下一輪派工，本輪不做）

1. **實機素材錄製（sideworker）**：依據本清單第五節之 REC-01 至 REC-05 操作序列，使用 `/root/gameplay_capture.sh` 或 Godot 測試腳本錄製 5 段乾淨的無壓縮 60fps 畫面片段。
2. **首幀素材生成（需經確認）**：依據第六節 9:16 提示詞使用 `gen_media.py` 產出 Shot 1 封面圖（需帶 `--ref branding/key_visual_main.png`）。
3. **剪接合成**：使用 ffmpeg 依據第四節時間軸（30.0s）進行接鏡與實體音效混音，產出最終 1080×1920 直式短影音。
