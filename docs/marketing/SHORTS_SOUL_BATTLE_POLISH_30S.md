...
Shot 5 (20.5-27.0s)【實機·高潮】怒氣滿 100%，一指觸發「🔥 暴怒覺醒！」超轉速狂暴連斬
        │ (最後一記重劈震屏白閃，瞬間切純黑)
...
| **Shot 2<br>【單手熱區】** | `3.0-7.5s`<br>(4.5s) | 鏡頭穿透光芒無縫切入實機戰鬥。小白 vs 荒路殘兵。展示神殿荒路背景與右側「右手拇指操作熱區」（`ThumbPad`）。玩家虛擬指尖輕點右下角 `ThumbAttack`（攻擊）連續出招，隨即點擊 `ThumbSwitch` 順暢切換巨錘。 | 平視直式實機展示視角向操作熱區與戰鬥動態平滑推鏡推進（Push In 115%），畫面居中，展現單手操作反饋。 | 乾脆俐落的機械按鍵點擊音「噠、噠」，長劍出鞘破空聲「颼——」，武器切換時齒輪彈射嵌合聲「咔嚓」。 | 【一指打完整場】<br>右手拇指熱區 · 普攻換武秒切換 | **【實機素材·REC-01】**<br>錄製自 Godot 實機 `res://scenes/battle/battle.tscn`。<br>展示已合併 main 的 `_ensure_thumb_hud()` 右手操作熱區。 |
| **Shot 3<br>【打擊換血】** | `7.5-15.5s`<br>(8.0s) | 小白正面交鋒守衛巨獸雷歐。畫面細節特寫：告別 16x16 粗像素方塊，呈現通透平滑插畫感。角色身上帶有深暖褐粗描邊（`outline.gdshader`），腳底踩著清晰貼地的獨立柔化橢圓軟影（`foot_shadow.gdshader`）。長劍斬中，觸發精確 0.15s 打擊停頓（Hitstop），爆出金色齒輪火花與微震反饋。 | 平視特寫鏡頭隨戰鬥節奏平滑向前推入特寫（Push In 118%），特寫展現受擊停頓、粗描邊與落地軟影銜接。 | 沉重鈍擊與金屬切割交錯聲「鏘——轟！」，0.15s 命中停頓瞬間聲場短暫抽真空，隨後火花炸裂。 | 【全新打擊手感】<br>去像素插畫感 × 落地柔影 × 0.15s 停頓 | **【實機素材·REC-02】**<br>錄製自 Godot 實機 `res://scenes/battle/battle.tscn`（`setup("leo")` 或 `setup("road_bandit")`）。<br>展示已合併 main 的 `game/shaders/outline.gdshader`、`game/shaders/foot_shadow.gdshader` 與 `capture_battle_polish.gd`。 |
| **Shot 4<br>【封靈開光】** | `15.5-20.5s`<br>(5.0s) | 戰鬥火花化為聚魂殿金光。展示四大殿堂之「聚魂殿」。中央展示綠、藍、紫、橙四階發條封靈罐（無任何系統 Emoji）。首屏上方直觀透明標明「保底進度 60/100 · 再抽 4 次獲得碎片」，機制清楚透明。 | 鏡頭自全景向中央封靈罐平滑推入（Push In 125%），聚焦展示首屏保底進度條與四階金屬封靈罐。 | 發條罐劇烈上弦發條聲「嘸——咔啦啦」，金屬鎖扣彈開聲，開光爆發的清脆天籟鐘鳴「鐺——」。 | 【透明首屏保底】<br>發條封靈罐 · 告別隱形坑 | **【實機素材·REC-03】**<br>錄製自 Godot 實機 `res://scenes/main.tscn` 之 `_go_soul_panel()`。<br>展示已合併 main 的首屏保底進度條與封靈罐系統（無 Emoji 現代 UI）。 |
| **Shot 5<br>【過載狂斬】** | `20.5-27.0s`<br>(6.5s) | 能量注入，畫面急切回戰鬥現場！怒氣累積滿 100%，畫面中央上方觸發「暴怒覺醒！」橘紅跳字與「暴怒中 · 屬性提升」狀態。小白攻擊速度與傷害提升，展開突進三連斬擊連續命中雷歐，雷歐血量自 420 扣減至 369，戰鬥日誌即時結算 3 次 17 點暴擊傷害！ | 鏡頭拉近平視動態跟隨，刀光突刺伴隨節奏微震與受擊閃爍，展現高潮絕殺力度。 | 高壓蒸氣劇烈排氣「嘶——！」，發條超轉速蜂鳴，連續斬擊的金屬暴擊「乒乒乓乓！」。 | 【超轉速過載】<br>暴怒覺醒 · 一指逆轉戰局！ | **【實機素材·REC-04】**<br>錄製自 Godot 實機 `res://scenes/battle/battle.tscn`。<br>展示已合併 main 的怒氣暴怒覺醒（`trigger_fury_awakening`）「暴怒覺醒！」跳字與連續三段斬擊命中。 |
...
### 【實機素材 4：REC-04 怒氣滿額暴怒覺醒與三連斬擊】
- **對應分鏡**：Shot 5 (20.5s～27.0s)
- **所屬功能**：`mob-battle` 與 `balance-lock`（暴怒覺醒，main 合併 commit `985ba28`）
...
- **錄製操作序列**：
  1. 載入雷歐戰鬥，預充怒氣值至 100%（`player.rage = 100.0`）。
  2. 觸發暴怒覺醒，畫面跳出「暴怒覺醒！」橘紅跳字，怒氣槽標註「暴怒中 · 屬性提升」。
  3. 小白以手動暴怒攻擊加成展開連續突刺三斬擊，連續 3 次命中守衛泰坦·雷歐。
  4. 雷歐血量由 420 扣減至 369，戰鬥日誌連續記錄 3 筆 17 點暴擊傷害。
  5. 獅衛重盾完好在手（無部位破壞演出），畫面乾淨無任何系統 emoji。
- **預期畫面**：戰鬥高潮、多巴胺打擊反饋拉滿，怒氣滿額暴怒覺醒與連斬命中的打擊快感。
...
|---|---|---|---|
| **第 10 條 / 第 10c 條** | **畫面真的在動（PSNR < 50dB）** | 拒絕靜圖硬切與假動態。全片 6 鏡皆具備真實動態或運鏡，依 review.md 第 10c 條每鏡取首中末三幀兩兩比對 PSNR，全數介於 11.34 dB ～ 29.71 dB（遠低於 50dB 門檻，查驗數據見 `docs/marketing/SHORTS_REC_INDEX.md` 第六節）。 | ✅ 合格 |
...
| **第 14 條 / 第 14a 條** | **實體音優先，成片包含完整實體音軌** | `ffprobe -select_streams a` 查驗包含 aac 44100Hz 立體聲音訊串流（30.000000s）。全片無罐頭交響樂，六鏡逐鏡對點實體機械採樣（發條咬合「喀——嗒！」、按鍵「噠」、破空「颼」、齒輪嵌合「咔嚓」、重擊「鏘——轟！」、上弦「嘸——咔啦啦」、鎖扣彈開、天籟鐘鳴「鐺——」、蒸氣「嘶——」、暴怒連斬暴擊、片尾單聲「喀嗒」後靜默）。 | ✅ 合格 |
...
| **第 19f 條** | **自我檢查表打勾要有根據** | 查驗均有確鑿執行依據：音軌經 ffprobe 查證為 aac 立體聲；PSNR 動態經 ffmpeg 逐鏡 3 點 pairwise 全解析度比對全數通過（<50dB）；實機檔案逐條 ls 通過；跳字經 grep 與 vision 雙重查證，絕無虛打勾。 | ✅ 合格 |
