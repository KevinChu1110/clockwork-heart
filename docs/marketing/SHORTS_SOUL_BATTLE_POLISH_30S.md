...
- **對應腳本**：`game/scripts/systems/soul_system.gd`、`game/scripts/ui/main.gd`
...
| **第 19b 條** | **標實機的必須真的是 Godot 錄影** | 實機清單列出精確存在之 Godot 場景與著色器路徑（`game/shaders/outline.gdshader`、`game/shaders/foot_shadow.gdshader`、`scenes/battle/battle.tscn`、`scenes/main.tscn`），REC-03 註明手動操作序列，絕無虛構腳本路徑。 | ✅ 合格 |
